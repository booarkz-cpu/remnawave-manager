#!/usr/bin/env bash
# ==============================================================================
# Remnawave Manager v25.1.13-prod
...
write_firewall(){
  ...
  ssh_list="$(ssh_port_list)"
  if [[ $DRY_RUN -eq 1 ]]; then
    log "[DRY-RUN] UFW: SSH $ssh_list/tcp, 80/tcp, 443/tcp; Single: Node 2222 только с Docker-сети; Edge: 2222 только с Panel IP; UDP/8443 если Hysteria2"
    return 0
  fi
...
create_subscription_token(){
  ...
  if [[ -z "$SUB_API_TOKEN" || "$SUB_API_TOKEN" == null ]]; then
    warn 'Минимальные scopes для Subscription Page отклонены backend; создаём token со scopes=["*"].'
    SUB_API_TOKEN="$(create_named_token remna-subpage '["*"]')"
  fi
  [[ -n "$SUB_API_TOKEN" && "$SUB_API_TOKEN" != null ]] || die 'API token для Subscription Page не получен.'
  persist_manager SUB_API_TOKEN "$SUB_API_TOKEN"
}
...
migrate_ssl_params_includes(){
  ...
  done < <(grep -Rl --include='*.conf' 'include /etc/nginx/conf.d/ssl-params.conf' /etc/nginx 2>/dev/null || true)
}

tls_params(){
  mkdir -p /etc/nginx/snippets
  # ssl-params.conf in conf.d/ is auto-included by Ubuntu and then re-included
  # from vhosts, which duplicates TLSv1.2/1.3 and inflates proxy_headers_hash.
  # Rewrite leftover includes BEFORE deleting the file, otherwise nginx -t
  # fails mid-repair (reality-site.conf still points at the old path).
...
repair(){
  ...
  [[ $DRY_RUN -eq 1 ]] && { log '[DRY-RUN] repair: nginx proxy-заголовки, vhosts, SelfSteal'; return 0; }
  NGINX_SKIP_RELOAD=1
  tls_params
  set_gzip
  case "${MODE:-single}" in
...
EOF2
  ...
  NGINX_SKIP_RELOAD=0
  nginx_apply
  ok 'Nginx, proxy-заголовки и маскировочный сайт обновлены. Проверьте панель, подписку и SNI-сайт.'
}
...
Параметры:
  ...
  --selfsteal-template N базовый шаблон маскировочного сайта: corgi|simple|business|nothing
...
Диагностика и обслуживание:
  $0 status
  $0 doctor
  $0 repair              исправить nginx/502 и маскировочный SNI-сайт на уже установленном VDS
  $0 backup
  $0 update
  $0 restore FILE
  $0 uninstall
...
main(){
  ...
  case "${1:-install}" in
    install)
      ...
      wizard "$mode"
      if [[ $DRY_RUN -eq 1 ]]; then
        echo "ПРОБНЫЙ ЗАПУСК: режим=$mode panel=${DOMAIN_PANEL:-} sub=${DOMAIN_SUB:-} reality=${DOMAIN_REALITY:-} hysteria2=$HYSTERIA2 monitoring=$MONITORING telegram=$TELEGRAM_ALERTS bbr=$BBR ipv6_отключён=$DISABLE_IPV6 admin=${ADMIN_USERNAME:-}"
        install_base
        configure_security
        configure_kernel
        write_firewall "$mode"
