...
# Keys that must never be overwritten from manager.env or CLI KEY=VALUE.
is_protected_kv_key(){
  case "${1:-}" in
    VERSION|PATH|HOME|IFS|PWD|OLDPWD|EUID|UID|PPID|TMPDIR|CDPATH|BASH_ENV|ENV|SHELLOPTS|BASHOPTS|BASH_XTRACEFD|LD_PRELOAD|LD_LIBRARY_PATH|LD_AUDIT|BASH_SOURCE|BASH_LINENO|FUNCNAME|PIPESTATUS|BASH_VERSINFO|PS1|PS2|PS4|LOG|BASE|EDGE_BASE|ENV_FILE|DRY_RUN|AUTO_YES|API_LOCAL|SELF_UPDATE_URL)
      return 0 ;;
    *) return 1 ;;
  esac
}
...
load_kv_file(){
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    # Never let manager.env clobber the installer or the shell (Ubuntu VERSION=, PATH=, IFS=).
    is_protected_kv_key "$key" && continue
    val="${val#\"}"; val="${val%\"}"
    val="${val#\'}"; val="${val%\'}"
    printf -v "$key" '%s' "$val"
    export "$key"
...
normalize_runtime_flags(){
  HYSTERIA2="$(coerce_01 "${HYSTERIA2:-0}")"
  ENABLE_GRPC="$(coerce_01 "${ENABLE_GRPC:-0}")"
  ENABLE_XHTTP="$(coerce_01 "${ENABLE_XHTTP:-0}")"
  PROTOCOLS_EXPLICIT="$(coerce_01 "${PROTOCOLS_EXPLICIT:-0}")"
  TELEGRAM_ALERTS="$(coerce_01 "${TELEGRAM_ALERTS:-0}")"
  BBR="$(coerce_01 "${BBR:-1}")"
  DISABLE_IPV6="$(coerce_01 "${DISABLE_IPV6:-0}")"
...
t(){
  ...
  if [[ "${RW_LANG:-ru}" == en ]]; then
    case "$k" in
      ...
      lang_en) s='English' ;;
      lang_ru) s='Русский' ;;
      lang_hint) s='Enter keeps the current language. Item 22 switches later.' ;;
      ...
      m28) s='Users' ;;
      d28) s='List, create (expiry / traffic / devices), enable/disable, subscription URL — not the admin password' ;;
      m29) s='Node control' ;;
      ...
      help_users) s='  %s users list|create NAME [DAYS|YYYY-MM-DD] [GB|512M|10G] [DEVICES] | enable|disable|sub' ;;
      ...
      usr_expire) s='Subscription days (1–36500) or YYYY-MM-DD [365]: ' ;;
      usr_traffic) s='Traffic limit (0=unlimited, integer=GB, or 512M/10G/1T) [0]: ' ;;
      usr_devices) s='Device limit (0=no HWID cap, 1–1000) [0]: ' ;;
      usr_expire_bad) s='Bad expiry. Use 1–36500 days or YYYY-MM-DD.' ;;
      usr_traffic_bad) s='Bad traffic limit. Use 0, an integer (GB), or 512M/10G/1T.' ;;
      usr_devices_bad) s='Bad device limit. Use 0 (no cap) or 1–1000.' ;;
      usr_limits) s='Expiry %s  traffic %s  devices %s' ;;
      ...
      stub_title) s='Subscription domain stub' ;;
      stub_1) s='  [1] Status' ;;
      stub_2) s='  [2] Enable Corgi kennel on https://SUB/' ;;
      stub_3) s='  [3] Disable (root of SUB proxies to subscription-page again)' ;;
      stub_4) s='  [4] Refresh pages and photos' ;;
      stub_0) s='  [0] Back' ;;
      stub_on) s='On: https://%s/ shows the Corgi Lusi kennel. Subscription links /shortUuid still work.' ;;
      stub_off) s='Off: https://%s/ is the Remnawave subscription-page root.' ;;
      stub_none) s='No SUB domain on this VPS. Install the panel first.' ;;
      stub_ok) s='Subscription stub written to %s and nginx reloaded.' ;;
      ...
      stub_http) s='  https://%s/ → HTTP %s' ;;
      stub_http_500) s='nginx 500 on the SUB root: 1.5.5 used alias on location = /. Rewriting the vhost.' ;;
      stub_migrating) s='Updating SUB-domain nginx: location = / must not use alias (HTTP 500 on nginx 1.24).' ;;
      stub_migrated) s='SUB kennel vhost rewritten. https://%s/ should open the kennel, not 500.' ;;
      stub_migrate_fail) s='Could not reload nginx after rewriting the SUB kennel vhost. Run: nginx -t && repair' ;;
      stub_write_fail) s='Could not write /var/www/sub-site; SUB root still proxies to subscription-page.' ;;
      doc_stub) s='Subscription stub' ;;
      ...
      done_secrets) s='Secrets (not printed here, not written to the installer log):' ;;
      done_login) s='Panel login once: menu 23 or  bash remnawave-manager.sh admin-login SHOW' ;;
      done_delete) s='Delete credentials.txt after you have saved the secrets.' ;;
      crt_fail) s='certbot failed. See the output above.' ;;
      status_title) s='=== Remnawave status %s ===' ;;
      help_title) s='Corgi Lusi · Remnawave Manager %s (English UI)' ;;
      help_menu) s='No arguments: choose language, then the interactive menu with a description of every function.' ;;
      ...
      help_lang) s='  %s --lang en|ru           skip the startup language picker (saved in manager.env)' ;;
    ...
    case "$k" in
      ...
      lang_en) s='English' ;;
      lang_ru) s='Русский' ;;
      lang_hint) s='Enter — оставить текущий язык. Пункт 22 переключает позже.' ;;
      ...
      m28) s='Пользователи' ;;
      d28) s='Список, создать (срок / трафик / устройства), вкл/выкл, ссылка подписки — не пароль администратора' ;;
      ...
      help_users) s='  %s users list|create ИМЯ [ДНИ|ГГГГ-ММ-ДД] [ГБ|512M|10G] [УСТРОЙСТВА] | enable|disable|sub' ;;
      ...
      usr_expire) s='Срок подписки: дни (1–36500) или ГГГГ-ММ-ДД [365]: ' ;;
      usr_traffic) s='Лимит трафика (0=безлимит, целое=ГБ, или 512M/10G/1T) [0]: ' ;;
      usr_devices) s='Лимит устройств (0=без HWID, 1–1000) [0]: ' ;;
      usr_expire_bad) s='Плохой срок. Нужны дни 1–36500 или дата ГГГГ-ММ-ДД.' ;;
      usr_traffic_bad) s='Плохой лимит трафика. 0, целое (ГБ) или 512M/10G/1T.' ;;
      usr_devices_bad) s='Плохой лимит устройств. 0 (без ограничения) или 1–1000.' ;;
      usr_limits) s='Срок %s  трафик %s  устройства %s' ;;
      ...
      stub_title) s='Заглушка на домене подписки' ;;
      stub_1) s='  [1] Состояние' ;;
      stub_2) s='  [2] Включить питомник корги на https://SUB/' ;;
      stub_3) s='  [3] Выключить (корень SUB снова ведёт на subscription-page)' ;;
      stub_4) s='  [4] Обновить страницы и фото' ;;
      stub_0) s='  [0] Назад' ;;
      stub_on) s='Вкл: https://%s/ показывает питомник Corgi Lusi. Ссылки /shortUuid работают.' ;;
      stub_off) s='Выкл: https://%s/ — корень Remnawave subscription-page.' ;;
      stub_none) s='На этом VDS нет домена SUB. Сначала поставьте панель.' ;;
      stub_ok) s='Заглушка записана в %s, nginx перезагружен.' ;;
      ...
      stub_http) s='  https://%s/ → HTTP %s' ;;
      stub_http_500) s='nginx 500 на корне SUB: в 1.5.5 alias на location = /. Переписываю vhost.' ;;
      stub_migrating) s='Обновляю nginx домена SUB: location = / нельзя через alias (HTTP 500 на nginx 1.24).' ;;
      stub_migrated) s='Vhost питомника переписан. https://%s/ должен открывать сайт, не 500.' ;;
      stub_migrate_fail) s='Не удалось перезагрузить nginx после правки vhost питомника. Выполните: nginx -t && repair' ;;
      stub_write_fail) s='Не удалось записать /var/www/sub-site; корень SUB по-прежнему на subscription-page.' ;;
      doc_stub) s='Заглушка подписки' ;;
      ...
      done_secrets) s='Секреты (здесь и в лог установщика не печатаются):' ;;
      done_login) s='Вход в панель один раз: пункт 23 или  bash remnawave-manager.sh admin-login SHOW' ;;
      done_delete) s='Удалите credentials.txt после сохранения секретов.' ;;
      crt_fail) s='certbot не удалось. Смотрите вывод выше.' ;;
      status_title) s='=== Состояние Remnawave %s ===' ;;
      help_title) s='Корги Люси · Remnawave Manager %s (русский интерфейс)' ;;
      help_menu) s='Без аргументов: сначала язык, затем меню с описанием всех функций.' ;;
      ...
      help_lang) s='  %s --lang en|ru           пропустить выбор языка при старте (сохраняется в manager.env)' ;;
...
upsert_kv(){
  ...
  chmod 600 "$file"
  tmp="$(mktemp)"
  # ENVIRON: awk -v treats `\` in passwords as escape sequences.
  RW_UPSERT_KEY="$key" RW_UPSERT_VAL="$value" awk '
    BEGIN { k=ENVIRON["RW_UPSERT_KEY"]; v=ENVIRON["RW_UPSERT_VAL"]; found=0 }
    index($0, k "=") == 1 { print k "=" v; found=1; next }
    { print }
    END { if (!found) print k "=" v }
  ' "$file" > "$tmp"
  unset RW_UPSERT_KEY RW_UPSERT_VAL
  mv "$tmp" "$file"
  chmod 600 "$file"
}
...
choose_language(){
  ...
  echo "${U_DIM}$(t lang_hint)${U_RESET}"
  if [[ $AUTO_YES -eq 1 ]]; then
    choice="$def"
  else
    read -r -p "[$def]: " choice || true
    choice="${choice:-$def}"
  fi
  case "$choice" in
...
ask(){
  ...
  if [[ -n "${!var:-}" ]]; then val="${!var}"; else
    if [[ $AUTO_YES -eq 1 ]]; then
      ...
      read -r -p "${C_CYAN}?${C_RESET} $prompt [$default]: " val || true; val="${val:-$default}"
    else
      while :; do
        if ! read -r -p "${C_CYAN}?${C_RESET} $prompt: " val; then
          die "$(t err_cancelled)"
        fi
        [[ -n "$val" ]] && break
      done
...
check_os(){
  ...
  case "${os_id:-}" in ubuntu|debian) ;; *) die "$(t err_os "${os_id:-unknown}")";; esac
}
...
write_panel_env(){
  mkdir -p "$BASE"
  local pgpass appsecret webhooksecret metrics_pass oldpwd="$PWD"
  cd "$BASE" || die "$(t err_file "$BASE")"
  if [[ ! -f .env ]]; then
...
svc_logs(){
  ...
  if [[ -n "$name" ]]; then
    trap ':' INT
    docker logs --tail 100 -f "$name" || true
    trap - INT
    return 0
  fi
...
ask_protocols(){
  ...
  if [[ $AUTO_YES -eq 1 ]]; then
    ...
    read -r -p "$(t prompt_protocols)" choice || true
    choice="${choice:-5}"
  fi
  case "$choice" in
    1) HYSTERIA2=0; ENABLE_GRPC=0; ENABLE_XHTTP=0 ;;
    2) HYSTERIA2=1; ENABLE_GRPC=0; ENABLE_XHTTP=0 ;;
    3) HYSTERIA2=0; ENABLE_GRPC=1; ENABLE_XHTTP=0 ;;
    4) HYSTERIA2=0; ENABLE_GRPC=0; ENABLE_XHTTP=1 ;;
    *) enable_all_protocols ;;
  esac
  PROTOCOLS_EXPLICIT=1
  [[ $DRY_RUN -eq 1 ]] && return 0
...
fetch_sub_stub_photos(){
  local dest="$1" tmp tgz script_dir
  mkdir -p "$dest"
  script_dir="$(cd -- "$(dirname -- "$(script_path 2>/dev/null || true)")" 2>/dev/null && pwd)" || script_dir='/nonexistent-sub-stub'
  for tgz in \
      "$script_dir/assets/sub-stub-photos.tgz" \
      "$script_dir/sub-stub-photos.tgz" \
      "$ADDONS_DIR/sub-stub-photos.tgz"; do
    if [[ -f "$tgz" ]] && tar -tzf "$tgz" >/dev/null 2>&1; then
      tar -xzf "$tgz" -C "$dest" && [[ -f "$dest/hero.jpg" ]] && return 0
    fi
  done
  tmp="$(mktemp)"
  for url in "$SUB_STUB_TGZ_URL" "$JSDELIVR_STUB_TGZ_URL"; do
    if curl -fL --retry 2 --retry-all-errors --retry-delay 1 --connect-timeout 8 --max-time 25 \
         -A 'remnawave-manager' -o "$tmp" "$url" 2>/dev/null \
       && tar -tzf "$tmp" >/dev/null 2>&1; then
      tar -xzf "$tmp" -C "$dest"
      mkdir -p "$ADDONS_DIR" 2>/dev/null || true
      cp -f "$tmp" "$ADDONS_DIR/sub-stub-photos.tgz" 2>/dev/null || true
      rm -f "$tmp"
      [[ -f "$dest/hero.jpg" ]] && return 0
    fi
    rm -f "$tmp"
  done
  return 1
...
write_sub_stub_svg(){
  ...
  case "$kind" in
    hero)
      cat > "$dest" <<'EOF_SVG'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 420" role="img" aria-label="Corgi">
<rect width="640" height="420" rx="28" fill="#e8f0d8"/>
<ellipse cx="320" cy="360" rx="220" ry="28" fill="#c5d4a8"/>
<ellipse cx="340" cy="250" rx="130" ry="70" fill="#d38a3a"/>
<ellipse cx="340" cy="220" rx="100" ry="78" fill="#e0a25a"/>
<ellipse cx="292" cy="204" rx="28" ry="22" fill="#fff6ea"/><ellipse cx="388" cy="204" rx="28" ry="22" fill="#fff6ea"/>
<circle cx="300" cy="202" r="8" fill="#2b1d12"/><circle cx="380" cy="202" r="8" fill="#2b1d12"/>
<ellipse cx="340" cy="228" rx="12" ry="9" fill="#2b1d12"/>
<path d="M288 148 256 88 312 138z" fill="#c45c26"/><path d="M392 148 436 84 368 138z" fill="#c45c26"/>
<path d="M230 260c14 36 38 56 64 60" stroke="#9a5a28" stroke-width="16" fill="none" stroke-linecap="round"/>
<path d="M450 260c-10 36-30 56-56 62" stroke="#9a5a28" stroke-width="16" fill="none" stroke-linecap="round"/>
</svg>
EOF_SVG
      ;;
    puppy)
      cat > "$dest" <<'EOF_SVG'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 420 420" role="img" aria-label="Puppy">
<rect width="420" height="420" rx="28" fill="#f3e2c9"/>
<ellipse cx="210" cy="250" rx="96" ry="78" fill="#e8b56a"/>
<circle cx="210" cy="188" r="72" fill="#f0c27a"/>
<ellipse cx="176" cy="176" rx="22" ry="18" fill="#fff6ea"/><ellipse cx="244" cy="176" rx="22" ry="18" fill="#fff6ea"/>
<circle cx="182" cy="174" r="7" fill="#2b1d12"/><circle cx="238" cy="174" r="7" fill="#2b1d12"/>
<ellipse cx="210" cy="198" rx="10" ry="8" fill="#2b1d12"/>
<path d="M168 132 142 78 188 124z" fill="#c45c26"/><path d="M252 132 286 76 232 124z" fill="#c45c26"/>
<ellipse cx="210" cy="330" rx="70" ry="18" fill="#d8c4a4"/>
</svg>
EOF_SVG
      ;;
    walk)
      cat > "$dest" <<'EOF_SVG'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 420" role="img" aria-label="Walk">
<rect width="640" height="420" rx="28" fill="#dce8c8"/>
<rect y="300" width="640" height="120" fill="#b7c89a"/>
<ellipse cx="300" cy="250" rx="120" ry="56" fill="#cf8a3c"/>
<ellipse cx="390" cy="220" rx="70" ry="54" fill="#e0a25a"/>
<path d="M360 180 340 128 380 170z" fill="#c45c26"/><path d="M420 184 458 126 400 174z" fill="#c45c26"/>
<circle cx="372" cy="214" r="6" fill="#2b1d12"/><circle cx="404" cy="214" r="6" fill="#2b1d12"/>
<path d="M210 260 190 330" stroke="#9a5a28" stroke-width="14" stroke-linecap="round"/>
<path d="M250 268 240 336" stroke="#9a5a28" stroke-width="14" stroke-linecap="round"/>
<path d="M340 268 354 338" stroke="#9a5a28" stroke-width="14" stroke-linecap="round"/>
<path d="M380 260 410 332" stroke="#9a5a28" stroke-width="14" stroke-linecap="round"/>
</svg>
EOF_SVG
      ;;
    *)
      cat > "$dest" <<'EOF_SVG'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 420" role="img" aria-label="Pair">
<rect width="640" height="420" rx="28" fill="#efe4d4"/>
<ellipse cx="250" cy="250" rx="110" ry="70" fill="#d38a3a"/>
<ellipse cx="400" cy="260" rx="100" ry="64" fill="#7a4a2a"/>
<ellipse cx="400" cy="260" rx="40" ry="28" fill="#f3e2c9"/>
<circle cx="230" cy="230" r="7" fill="#2b1d12"/><circle cx="270" cy="230" r="7" fill="#2b1d12"/>
<circle cx="380" cy="244" r="6" fill="#2b1d12"/><circle cx="416" cy="244" r="6" fill="#fff6ea"/>
</svg>
EOF_SVG
      ;;
  esac
...
write_sub_stub_site(){
  local site="${SUB_SITE:-/var/www/sub-site}"
  mkdir -p "$site/img"
  write_sub_stub_svg "$site/img/hero.svg" hero
  write_sub_stub_svg "$site/img/puppy.svg" puppy
  write_sub_stub_svg "$site/img/walk.svg" walk
  write_sub_stub_svg "$site/img/pair.svg" pair
  if fetch_sub_stub_photos "$site/img"; then
    ok "$(t stub_photos_ok "$site/img")"
  else
    warn "$(t stub_photos_miss)"
  fi
  cat > "$site/favicon.svg" <<'EOF_SITE'
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><rect width="64" height="64" rx="14" fill="#c45c26"/><ellipse cx="32" cy="38" rx="16" ry="12" fill="#f3e2c9"/><circle cx="26" cy="36" r="3" fill="#2b1d12"/><circle cx="38" cy="36" r="3" fill="#2b1d12"/><path d="M20 22 12 8 26 20z" fill="#e0a25a"/><path d="M44 22 52 8 38 20z" fill="#e0a25a"/></svg>
EOF_SITE
  printf 'User-agent: *\nAllow: /\n' > "$site/robots.txt"
...
.hero{display:grid;grid-template-columns:1.05fr .95fr;gap:32px;align-items:center;padding:40px 0}
h1{font-size:42px;line-height:1.15;margin:0 0 12px}h2{font-size:26px;margin:0 0 8px}
.card{background:var(--card);border:1px solid var(--line);border-radius:18px;padding:18px;box-shadow:0 10px 30px rgba(43,29,18,.05)}
...
.photo{width:100%;height:auto;border-radius:16px;display:block;background:#f3e2c9}
.caption{color:var(--muted);font-size:14px;margin:8px 0 0}
footer{color:var(--muted);font-size:14px;padding:40px 0 64px}
@media (max-width:800px){.hero{grid-template-columns:1fr;padding:24px 0}h1{font-size:32px}}
EOF_SITE
...
<meta name="description" content="Corgi Lusi — семейный питомник вельш-корги пемброк. Щенки, взрослые собаки, уход и фотографии.">
  ...
  <div class="wrap">
    ...
    <section class="hero">
      <div>
        ...
        <p>Corgi Lusi — домашний питомник. Мы растим здоровых корги, снимаем их жизнь и делимся советами по уходу, питанию и воспитанию.</p>
        <p><a class="btn" href="/gallery.html">Смотреть фото</a></p>
      </div>
      <figure class="card">
        <img class="photo" src="/img/hero.jpg" alt="Вельш-корги на траве" width="800" height="600" onerror="this.onerror=null;this.src='/img/hero.svg'">
        <figcaption class="caption">Утро в саду — Lusi следит за участком, как подобает пастуху.</figcaption>
      </figure>
    </section>
  </div>
</header>
<main class="wrap">
  <section class="grid">
    <article class="card">
      <img class="photo" src="/img/puppy.jpg" alt="Щенок корги" width="640" height="640" onerror="this.onerror=null;this.src='/img/puppy.svg'">
      <h2>Щенки</h2>
      <p>Помёты редкие. Перед щенком говорим о спине, весе и том, что корги — рабочая собака, а не диванная игрушка.</p>
    </article>
    <article class="card">
      <img class="photo" src="/img/walk.jpg" alt="Корги на прогулке" width="800" height="600" onerror="this.onerror=null;this.src='/img/walk.svg'">
      <h2>Прогулки</h2>
      <p>Короткие ноги — не повод пропускать лес. Нужны задачи, нюхательные маршруты и спокойный темп.</p>
    </article>
    <article class="card">
      <img class="photo" src="/img/pair.jpg" alt="Два корги дома" width="800" height="600" onerror="this.onerror=null;this.src='/img/pair.svg'">
      <h2>Дом</h2>
      <p>Два корги — это хор и комитет. Им нужны правила, место на диване и человек, который не сдаётся первым.</p>
    </article>
  </section>
</main>
<footer class="wrap"><p>© Corgi Lusi. Питомник вельш-корги: фото, уход и жизнь с пастушьей собакой в городе.</p></footer>
</body></html>
EOF_SITE
  cat > "$site/about.html" <<'EOF_SITE'
<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>О питомнике — Corgi Lusi</title><link rel="icon" href="/favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="/styles.css"></head>
<body><header><div class="wrap"><nav><a class="brand" href="/">Corgi Lusi</a><a href="/about.html">О питомнике</a><a href="/breeds.html">Порода</a><a href="/gallery.html">Фото</a><a href="/contact.html">Контакты</a></nav></div></header>
<main class="wrap"><article class="card"><img class="photo" src="/img/pair.jpg" alt="Корги питомника" width="800" height="600" onerror="this.onerror=null;this.src='/img/pair.svg'"><h1>О питомнике</h1><p>Lusi — домашняя линия вельш-корги пемброк. Мы не гонимся за количеством помётов: важны здоровье, социализация и честный рассказ о породе.</p><p>Гостей принимаем по записи. На фото — наши собаки, не стоковые картинки из каталога.</p></article></main>
<footer class="wrap"><p>© Corgi Lusi</p></footer></body></html>
EOF_SITE
  cat > "$site/breeds.html" <<'EOF_SITE'
<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Порода — Corgi Lusi</title><link rel="icon" href="/favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="/styles.css"></head>
<body><header><div class="wrap"><nav><a class="brand" href="/">Corgi Lusi</a><a href="/about.html">О питомнике</a><a href="/breeds.html">Порода</a><a href="/gallery.html">Фото</a><a href="/contact.html">Контакты</a></nav></div></header>
<main class="wrap"><article class="card"><img class="photo" src="/img/walk.jpg" alt="Корги в лесу" width="800" height="600" onerror="this.onerror=null;this.src='/img/walk.svg'"><h1>Вельш-корги пемброк</h1><p>Пемброк легче кардигана и обычно короткохвост. Обе разновидности — пастухи и обе склонны к набору веса.</p><p>На площадке корги любит контроль: следит за детьми, велосипедами и чужими собаками. Это работа, которую нужно направлять.</p></article></main>
<footer class="wrap"><p>© Corgi Lusi</p></footer></body></html>
EOF_SITE
  cat > "$site/gallery.html" <<'EOF_SITE'
<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Фото — Corgi Lusi</title><link rel="icon" href="/favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="/styles.css"></head>
<body><header><div class="wrap"><nav><a class="brand" href="/">Corgi Lusi</a><a href="/about.html">О питомнике</a><a href="/breeds.html">Порода</a><a href="/gallery.html">Фото</a><a href="/contact.html">Контакты</a></nav></div></header>
<main class="wrap"><h1>Фотогалерея</h1>
<section class="grid">
  <figure class="card"><img class="photo" src="/img/hero.jpg" alt="Корги на траве" width="800" height="600" onerror="this.onerror=null;this.src='/img/hero.svg'"><figcaption class="caption">Сад</figcaption></figure>
  <figure class="card"><img class="photo" src="/img/puppy.jpg" alt="Щенок корги" width="640" height="640" onerror="this.onerror=null;this.src='/img/puppy.svg'"><figcaption class="caption">Щенок</figcaption></figure>
  <figure class="card"><img class="photo" src="/img/walk.jpg" alt="Прогулка" width="800" height="600" onerror="this.onerror=null;this.src='/img/walk.svg'"><figcaption class="caption">Лес</figcaption></figure>
  <figure class="card"><img class="photo" src="/img/pair.jpg" alt="Два корги" width="800" height="600" onerror="this.onerror=null;this.src='/img/pair.svg'"><figcaption class="caption">Дом</figcaption></figure>
</section></main>
<footer class="wrap"><p>© Corgi Lusi</p></footer></body></html>
EOF_SITE
  cat > "$site/contact.html" <<'EOF_SITE'
<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Контакты — Corgi Lusi</title><link rel="icon" href="/favicon.svg" type="image/svg+xml"><link rel="stylesheet" href="/styles.css"></head>
<body><header><div class="wrap"><nav><a class="brand" href="/">Corgi Lusi</a><a href="/about.html">О питомнике</a><a href="/breeds.html">Порода</a><a href="/gallery.html">Фото</a><a href="/contact.html">Контакты</a></nav></div></header>
<main class="wrap"><article class="card"><img class="photo" src="/img/puppy.jpg" alt="Щенок корги" width="640" height="640" onerror="this.onerror=null;this.src='/img/puppy.svg'"><h1>Контакты</h1><p>Пишите на почту питомника. Мы отвечаем в рабочие дни и не продаём щенков «в один клик».</p><p>Экскурсии — только после короткого созвона: так спокойнее собакам.</p></article></main>
<footer class="wrap"><p>© Corgi Lusi</p></footer></body></html>
EOF_SITE
  chmod -R a+rX "$site"
  persist_manager SUB_STUB "${SUB_STUB:-1}" 2>/dev/null || true
  return 0
}

sub_stub_nginx_locations(){
  # root + try_files. Never `alias` a file on `location = /`: nginx 1.24 then
  # opens ".../index.htmlindex.html" (ENOTDIR) and returns HTTP 500.
  cat <<'EOF'
  root /var/www/sub-site;
  location = / { try_files /index.html =404; }
  location = /index.html { try_files /index.html =404; }
  location = /about.html { try_files /about.html =404; }
  location = /breeds.html { try_files /breeds.html =404; }
  location = /gallery.html { try_files /gallery.html =404; }
  location = /contact.html { try_files /contact.html =404; }
...
write_panel_vhosts(){
  local stub='' site="${SUB_SITE:-/var/www/sub-site}"
  if [[ ${SUB_STUB:-1} -eq 1 ]]; then
    if write_sub_stub_site && [[ -f "$site/index.html" ]]; then
      stub="$(sub_stub_nginx_locations)"
    else
      warn "$(t stub_write_fail)"
    fi
  fi
...
install_xray_checker(){
  ...
  [[ "$interval" =~ ^[0-9]+$ ]] && (( 10#$interval >= 30 && 10#$interval <= 86400 )) || die 'CHECK_INTERVAL: 30..86400.'
  mkdir -p "$root"
...
download_upstream(){
  ...
  if [[ "$url" =~ https://github.com/([^/]+/[^/]+)/raw/main/(.*)$ ]]; then
    ...
    curl -fsSL --max-time 90 -o "$out" "https://cdn.jsdelivr.net/gh/${repo}@main/${path}" && return 0
    return 1
  fi
  return 1
}
...
community_update_menu(){
  ...
  read -r -p "$(t prompt_choice)" choice || true
  case "$choice" in
    1) require_root; menu_call refresh_community_modules ;;
...
check_dns(){
  local d="$1" required="${2:-1}" ip answers
  answers="$(getent ahostsv4 "$d" | awk '{print $1}' | sort -u || true)"
  if [[ -z "$answers" ]]; then
    if [[ "$required" != 0 ]]; then
      die "DNS A-запись для $d отсутствует."
    fi
    warn "DNS A-запись для $d отсутствует."
    return 1
  fi
  ip="$(public_ip)"
...
preflight(){
  ...
  case "$mode" in
    panel)
      ...
      [[ -n "${DOMAIN_REALITY:-}" ]] && check_dns "$DOMAIN_REALITY" 0 || true
      ;;
    edge|node)
...
# Isolate die/ERR: exit inside a function only ends this subshell, not the menu.
menu_call(){ ( "$@" ) || true; }

os_release_field(){
  local key="$1"
  [[ -r /etc/os-release ]] || return 1
  awk -F= -v k="$key" '$1==k{v=substr($0,index($0,"=")+1); gsub(/^["'\'']|["'\'']$/,"",v); print v; exit}' /etc/os-release
}
...
# Menu counterpart of require_panel_api: print and return 1 instead of aborting the script.
panel_api_ready(){
  detect_install_mode
  if [[ "${MODE:-single}" == edge ]]; then
    echo "$(t err_need_panel)"
    return 1
  fi
  if already_installed || [[ -f "$BASE/docker-compose.yml" ]]; then
    bootstrap_auth
    return 0
  fi
  echo "$(t err_repair_no_install)"
  return 1
}
...
users_pick(){
  ...
  if [[ "$choice" =~ ^[1-9][0-9]*$ ]] && (( 10#$choice >= 1 && 10#$choice <= ${#uuids[@]} )); then
...
# Days 1–36500 or YYYY-MM-DD → ISO-8601 UTC. Empty / 0 → USER_EXPIRE_DAYS or 365.
user_expire_iso(){
  local raw="${1:-}" days iso
  raw="$(printf '%s' "$raw" | tr -d '[:space:]')"
  [[ -z "$raw" || "$raw" == 0 ]] && raw="$(printf '%s' "${USER_EXPIRE_DAYS:-365}" | tr -d '[:space:]')"
  [[ -z "$raw" ]] && raw=365
  if [[ "$raw" =~ ^[1-9][0-9]{0,4}$ ]] && (( 10#$raw >= 1 && 10#$raw <= 36500 )); then
    days="$raw"
    iso="$(date -u -d "+${days} days" +%Y-%m-%dT%H:%M:%S.000Z 2>/dev/null \
      || date -u -v+"${days}"d +%Y-%m-%dT%H:%M:%S.000Z 2>/dev/null)" || return 1
    [[ -n "$iso" ]] || return 1
    printf '%s' "$iso"
    return 0
  fi
  if [[ "$raw" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}$ ]]; then
    iso="$(date -u -d "${raw} 23:59:59" +%Y-%m-%dT%H:%M:%S.000Z 2>/dev/null \
      || date -u -j -f '%Y-%m-%d' "$raw" +%Y-%m-%dT23:59:59.000Z 2>/dev/null)" || return 1
    [[ -n "$iso" ]] || return 1
    printf '%s' "$iso"
    return 0
  fi
...
# 0 = unlimited. Bare integer = GiB. Suffixes: 512M, 10G, 1T (also MB/GB/TB).
user_traffic_bytes(){
  local raw n unit mul
  raw="$(printf '%s' "${1:-}" | tr '[:upper:]' '[:lower:]' | tr -d '[:space:]')"
  [[ -z "$raw" ]] && raw="$(printf '%s' "${USER_TRAFFIC_GB:-0}" | tr '[:upper:]' '[:lower:]' | tr -d '[:space:]')"
  case "$raw" in
    ''|0|unlimited|inf|none) printf 0; return 0 ;;
  esac
  if [[ "$raw" =~ ^[0-9]+$ ]]; then
    (( 10#$raw <= 1048576 )) || return 1
    printf '%s' "$((10#$raw * 1024 * 1024 * 1024))"
    return 0
  fi
  if [[ "$raw" =~ ^([0-9]+)(k|kb|kib|m|mb|mib|g|gb|gib|t|tb|tib)$ ]]; then
    n="${BASH_REMATCH[1]}"
    unit="${BASH_REMATCH[2]}"
    case "$unit" in
      k|kb|kib) mul=1024 ;;
      ...
      t|tb|tib) mul=$((1024 * 1024 * 1024 * 1024)) ;;
      *) return 1 ;;
    esac
    (( 10#$n >= 1 && 10#$n <= 1048576 )) || return 1
    printf '%s' "$((10#$n * mul))"
    return 0
  fi
  return 1
...
# 0 = omit hwidDeviceLimit (no cap). Else 1–1000.
user_device_limit(){
  local raw
  raw="$(printf '%s' "${1:-}" | tr -d '[:space:]')"
  [[ -z "$raw" ]] && raw="$(printf '%s' "${USER_DEVICE_LIMIT:-0}" | tr -d '[:space:]')"
  [[ -z "$raw" || "$raw" == 0 ]] && { printf 0; return 0; }
  [[ "$raw" =~ ^[1-9][0-9]{0,3}$ ]] || return 1
  (( 10#$raw >= 1 && 10#$raw <= 1000 )) || return 1
  printf '%s' "$raw"
}

users_payload(){
  local name="$1" expire="$2" bytes="$3" devices="$4"
  jq -nc \
    --arg u "$name" \
    --arg e "$expire" \
    --argjson b "$bytes" \
    ...
    + (if $s=="" then {} else {activeInternalSquads:[$s]} end)
    + (if $d==0 then {} else {hwidDeviceLimit:$d} end)'
}

users_create_named(){
  local name="$1" expire="$2" bytes="$3" devices="$4" body raw code json short url
  [[ "$name" =~ ^[A-Za-z][A-Za-z0-9._-]{2,31}$ ]] || die "$(t usr_bad)"
  [[ -n "$expire" ]] || die "$(t usr_expire_bad)"
  [[ "$bytes" =~ ^[0-9]+$ ]] || die "$(t usr_traffic_bad)"
  [[ "$devices" =~ ^[0-9]+$ ]] || die "$(t usr_devices_bad)"
  [[ -n "${SQUAD_UUID:-}" ]] || ensure_internal_squad || true
  body="$(users_payload "$name" "$expire" "$bytes" "$devices")"
  raw="$(api_call POST /users/ "$body" "$API_JWT" || true)"
  code="$(tail -n1 <<<"$raw")"
  json="$(sed '$d' <<<"$raw")"
  [[ "$code" =~ ^2[0-9][0-9]$ ]] || die "API HTTP $code"
  ...
  ok "$(t usr_limits "$expire" "$bytes" "$devices")"
}

users_create(){
  local name expire_in traffic_in devices_in expire bytes devices
  read -r -p "$(t usr_prompt)" name || true
  [[ "$name" =~ ^[A-Za-z][A-Za-z0-9._-]{2,31}$ ]] || { echo "$(t usr_bad)"; return 0; }
  read -r -p "$(t usr_expire)" expire_in || true
  expire="$(user_expire_iso "$expire_in")" || { echo "$(t usr_expire_bad)"; return 0; }
  read -r -p "$(t usr_traffic)" traffic_in || true
  bytes="$(user_traffic_bytes "$traffic_in")" || { echo "$(t usr_traffic_bad)"; return 0; }
  read -r -p "$(t usr_devices)" devices_in || true
  devices="$(user_device_limit "$devices_in")" || { echo "$(t usr_devices_bad)"; return 0; }
  users_create_named "$name" "$expire" "$bytes" "$devices"
}
...
users_cli(){
  local act="${1:-list}" name expire bytes devices
  shift || true
  require_root
  HYDRATE_QUIET=1 hydrate_install_state
  require_panel_api
  case "$act" in
    list|'') users_list_print ;;
    create)
      name="${1:-}"
      if [[ -z "$name" ]]; then users_create; else
        expire="$(user_expire_iso "${2:-}")" || die "$(t usr_expire_bad)"
        bytes="$(user_traffic_bytes "${3:-}")" || die "$(t usr_traffic_bad)"
        devices="$(user_device_limit "${4:-}")" || die "$(t usr_devices_bad)"
        users_create_named "$name" "$expire" "$bytes" "$devices"
      fi
      ;;
    enable|disable)
...
telegram_enable_interactive(){
  ...
  [[ -n "$token" && -n "$chat" ]] || { warn "$(t alb_test_fail)"; return 0; }
...
certs_renew_now(){
  if ! certbot renew --non-interactive --deploy-hook /usr/local/sbin/remnawave-nginx-reload.sh; then
    warn "$(t crt_fail)"
    return 0
  fi
  ...
  while true; do
    ...
    case "$choice" in
      ...
      2)
        ...
        validate_ip "$ip" || { echo "$(t err_invalid ADMIN_IP "$ip")"; continue; }
        ADMIN_IP="$ip"
        persist_manager ADMIN_IP "$ADMIN_IP"
        firewall_rebuild_now
        ;;
...
sub_stub_alias_broken(){
  local conf=/etc/nginx/conf.d/remnawave-web.conf
  [[ -f "$conf" ]] || return 1
  grep -Fq 'alias /var/www/sub-site/index.html' "$conf"
}

sub_stub_http_code(){
  local host="${1:-${DOMAIN_SUB:-}}" code
  [[ -n "$host" ]] || { printf '000'; return 0; }
  code="$(probe_http_code "https://${host}/" -k --resolve "${host}:443:127.0.0.1")"
  if [[ "$code" == '000' ]]; then
    code="$(probe_http_code "https://${host}:9443/" -k --resolve "${host}:9443:127.0.0.1")"
  fi
  printf '%s' "$code"
}

maybe_fix_sub_stub_nginx(){
  [[ ${DRY_RUN:-0} -eq 1 ]] && return 0
  [[ ${EUID:-$(id -u)} -eq 0 ]] || return 0
  sub_stub_alias_broken || return 0
  [[ -n "${DOMAIN_SUB:-}" && -n "${DOMAIN_PANEL:-}" ]] || return 0
  [[ "${MODE:-single}" == edge ]] && return 0
  log "$(t stub_migrating)"
  write_panel_vhosts
  if ! nginx -t >/dev/null 2>&1; then
    warn "$(t stub_migrate_fail)"
    return 1
  fi
  if [[ "${NGINX_SKIP_RELOAD:-0}" != 1 ]]; then
    if systemctl is-active --quiet nginx 2>/dev/null; then
      systemctl reload nginx || { warn "$(t stub_migrate_fail)"; return 1; }
    else
      systemctl start nginx || { warn "$(t stub_migrate_fail)"; return 1; }
    fi
  fi
  ok "$(t stub_migrated "$DOMAIN_SUB")"
}
...
sub_stub_apply(){
  ...
  HYDRATE_QUIET=1 hydrate_install_state
  detect_install_mode
  if [[ "${MODE:-single}" == edge || -z "${DOMAIN_SUB:-}" ]]; then
    echo "$(t stub_none)"
    return 0
  fi
  SUB_STUB="$want"
  persist_manager SUB_STUB "$SUB_STUB"
  write_panel_vhosts
  [[ "${NGINX_SKIP_RELOAD:-0}" == 1 ]] || nginx_apply
  if [[ "$want" == 1 ]]; then
    ok "$(t stub_ok "${SUB_SITE:-/var/www/sub-site}")"
    ok "$(t stub_on "$DOMAIN_SUB")"
    code="$(sub_stub_http_code "$DOMAIN_SUB")"
    echo "$(t stub_http "$DOMAIN_SUB" "$code")"
    if [[ "$code" =~ ^5[0-9][0-9]$ ]]; then
      warn "$(t stub_http_500)"
    fi
  else
    ok "$(t stub_off_ok)"
    ok "$(t stub_off "$DOMAIN_SUB")"
  fi
}

sub_stub_status(){
  local code site="${SUB_SITE:-/var/www/sub-site}"
  HYDRATE_QUIET=1 hydrate_install_state
  detect_install_mode
  ...
  if [[ ${SUB_STUB:-1} -eq 1 ]]; then
    echo "$(t stub_on "$DOMAIN_SUB")"
    [[ -f "$site/index.html" ]] && echo "  $site"
    [[ -f "$site/img/hero.jpg" ]] && echo "$(t stub_photos_ok "$site/img")"
    if sub_stub_alias_broken; then
      warn "$(t stub_http_500)"
    fi
    code="$(sub_stub_http_code "$DOMAIN_SUB")"
    echo "$(t stub_http "$DOMAIN_SUB" "$code")"
  else
    echo "$(t stub_off "$DOMAIN_SUB")"
  fi
}
...
sub_stub_menu(){
  ...
  while true; do
    echo
    echo "${U_BLUE}${U_BOLD}$(t stub_title)${U_RESET}"
    sub_stub_status
    echo "$(t stub_1)"
    echo "$(t stub_2)"
    echo "$(t stub_3)"
...
sub_stub_cli(){
  local act="${1:-status}"
  case "$act" in
    on|enable|1) sub_stub_apply 1 ;;
    off|disable|0) sub_stub_apply 0 ;;
    refresh|rewrite) sub_stub_apply 1 ;;
    status|'' ) sub_stub_status ;;
    menu) sub_stub_menu ;;
    *) die "$(t err_unknown_item)" ;;
  esac
}
...
doctor(){
  ...
  echo '[Docker]'
  echo '[Система]'; echo "$(os_release_field PRETTY_NAME 2>/dev/null || echo unknown)"; docker version --format '{{.Server.Version}}' 2>/dev/null || echo FAIL
  ...
  echo "[$(t doc_stub)]"
  if [[ ${SUB_STUB:-1} -eq 1 && -n "${DOMAIN_SUB:-}" && "${MODE:-single}" != edge ]]; then
    echo "$(t stub_on "$DOMAIN_SUB")"
    if sub_stub_alias_broken; then
      warn "$(t stub_http_500)"
      maybe_fix_sub_stub_nginx || true
    fi
    echo "$(t stub_http "$DOMAIN_SUB" "$(sub_stub_http_code "$DOMAIN_SUB")")"
  elif [[ -n "${DOMAIN_SUB:-}" ]]; then
    echo "$(t stub_off "$DOMAIN_SUB")"
  else
    echo "$(t stub_none)"
  fi
...
restore(){
  ...
  tmp="$(mktemp -d)"
  # EXIT, not RETURN: RETURN also fires when nested warn()/ok() return and would delete tmp early.
  trap 'rm -rf -- "$tmp"' EXIT
  ...
  if [[ -f "$BASE/docker-compose.yml" && -n "$db_member" ]]; then
    (cd "$BASE" && docker compose up -d remnawave-db remnawave-redis)
    for _ in {1..60}; do
...
wizard(){
  ...
  case "$mode" in
    single|panel)
      ...
      if [[ "$mode" == panel ]]; then
        if [[ -z "${EDGE_ADDRESS:-}" && $AUTO_YES -eq 0 ]]; then read -r -p "$(t prompt_edge_ip)" EDGE_ADDRESS || true; fi
        if [[ -n "${EDGE_ADDRESS:-}" ]]; then
...
interactive_menu(){
  ...
  # Language first, before the numbered menu. --lang / --yes / non-TTY skip the picker.
  if [[ $AUTO_YES -eq 0 && $RW_LANG_CLI -eq 0 && -t 0 && -t 1 ]]; then
    choose_language
  fi
  HYDRATE_QUIET=1 hydrate_install_state
  maybe_fix_sub_stub_nginx || true
  ...
  while true; do
    ...
    read -r -p "$(t prompt_choice)" choice || true
    case "$choice" in
      1) menu_call menu_install single ;;
      2) menu_call menu_install panel ;;
      3) menu_call menu_install node ;;
      4) require_root; ask_protocols; menu_call apply_protocols; menu_pause ;;
      5) menu_call status; menu_pause ;;
      6) menu_call doctor; menu_pause ;;
      7) require_root; menu_call repair; menu_pause ;;
      8)
        read -r -p "$(t prompt_logs)" name || true
        menu_call svc_logs "${name:-}"
        ;;
      9) require_root; menu_call compose_action up; menu_pause ;;
      10) require_root; menu_call compose_action down; menu_pause ;;
      11) require_root; menu_call compose_action restart; ok "$(t services_restarted)"; menu_pause ;;
      12) require_root; menu_call backup; menu_pause ;;
      13)
        require_root
        read -r -p "$(t prompt_restore)" arch || true
        [[ -n "${arch:-}" ]] || { echo "$(t err_restore_arg)"; menu_pause; continue; }
        menu_call restore "$arch"
        menu_pause
        ;;
      14) require_root; menu_call update; menu_pause ;;
      15) require_root; menu_call uninstall; menu_pause ;;
      16)
        require_root
        read -r -p "$(t prompt_core)" core_choice || true
        core_choice="${core_choice:-1}"
        if [[ "$core_choice" == 2 ]]; then
          menu_call restore_xray_core "${BASE}/node"
        else
          menu_call install_xray_core "${BASE}/node" "${XCORE_SOURCE:-official}"
        fi
        menu_pause
        ;;
      17)
        ...
        echo "$(t addon_e)"
        echo "$(t addon_f)"
        read -r -p "$(t prompt_addon)" choice || true
        case "$choice" in
          a) menu_call install_upstream_addon remnawave ;;
          b) menu_call install_upstream_addon remnanode ;;
          c) menu_call install_upstream_addon selfsteal ;;
          d) menu_call install_upstream_addon wtm ;;
          e) menu_call install_upstream_addon netbird ;;
          f) menu_call install_upstream_addon egames ;;
          *) echo "$(t err_unknown_item)" ;;
        esac
        menu_pause
        ;;
      18) require_root; menu_call enable_panel_stealth; menu_pause ;;
      19) require_root; menu_call install_cli; menu_pause ;;
      20)
        echo "$CONVERTER_URL"
      ...
      21) usage; menu_pause ;;
      22) choose_language ;;
      23) menu_call urls_and_optional_admin; menu_pause ;;
      24) community_update_menu; menu_pause ;;
      25) menu_call node_transports_menu; menu_pause ;;
      26) script_update_menu; menu_pause ;;
      27) menu_call add_node_menu; menu_pause ;;
      28) menu_call users_menu; menu_pause ;;
      29) menu_call node_control_menu; menu_pause ;;
      30) menu_call alerts_backup_menu; menu_pause ;;
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      *=*)
        key="${1%%=*}"
        val="${1#*=}"
        if [[ "$key" =~ ^[A-Za-z_][A-Za-z0-9_]*$ ]] && ! is_protected_kv_key "$key"; then
          printf -v "$key" '%s' "$val"
          export "$key"
        fi
        shift;;
