#!/usr/bin/env bash
# ==============================================================================
# Remnawave Manager v25.2.0-prod
...
create_config_profile(){
  [[ -n "${REALITY_SHORT_ID:-}" ]] || REALITY_SHORT_ID="$(rand 16)"
  local existing profile created raw code
  ...
  if [[ -n "$PROFILE_UUID" ]]; then
    raw="$(api_call PATCH "/config-profiles/${PROFILE_UUID}" "$(jq -nc --arg n AUTO-PROFILE --argjson c "$profile" '{name:$n,config:$c}')" "$API_JWT" || true)"
    code="$(tail -n1 <<<"$raw")"
    if [[ ! "$code" =~ ^2[0-9][0-9]$ ]]; then
      raw="$(api_call PUT "/config-profiles/${PROFILE_UUID}" "$(jq -nc --arg n AUTO-PROFILE --argjson c "$profile" '{name:$n,config:$c}')" "$API_JWT" || true)"
      code="$(tail -n1 <<<"$raw")"
    fi
    [[ "$code" =~ ^2[0-9][0-9]$ ]] || die "Не удалось обновить AUTO-PROFILE (HTTP ${code:-000})."
    ok "Профиль AUTO-PROFILE обновлён (Reality$([[ $HYSTERIA2 -eq 1 ]] && echo ' + Hysteria2')$([[ $ENABLE_GRPC -eq 1 ]] && echo ' + gRPC')$([[ $ENABLE_XHTTP -eq 1 ]] && echo ' + xHTTP'))."
  else
  ...
  [[ -n "$PROFILE_UUID" ]] || die 'Config Profile UUID не получен.'
  existing="$(api_json "$(api_call GET /config-profiles/ '' "$API_JWT")")"
  INBOUND_UUID="$(jq -r --arg p "$PROFILE_UUID" '
    .response.configProfiles[]? | select(.uuid==$p) |
    ((.inbounds[]? | select(.tag=="VLESS_REALITY") | .uuid) // .inbounds[0].uuid)
  ' <<<"$existing" | head -1)"
...
create_node(){
  local address="$1" name="$2" existing body node inbound_uuids
  ...
  if [[ -n "$plugin_uuid" ]]; then
    body="$(jq -nc --arg n "$name" --arg a "$address" --arg p "$PROFILE_UUID" --argjson ibs "$inbound_uuids" --arg pl "$plugin_uuid" '{name:$n,address:$a,port:2222,countryCode:"XX",configProfile:{activeConfigProfileUuid:$p,activeInbounds:$ibs},activePluginUuid:$pl,isTrafficTrackingActive:false,trafficLimitBytes:0,notifyPercent:0,trafficResetDay:31,consumptionMultiplier:1.0}')"
  else
    body="$(jq -nc --arg n "$name" --arg a "$address" --arg p "$PROFILE_UUID" --argjson ibs "$inbound_uuids" '{name:$n,address:$a,port:2222,countryCode:"XX",configProfile:{activeConfigProfileUuid:$p,activeInbounds:$ibs},isTrafficTrackingActive:false,trafficLimitBytes:0,notifyPercent:0,trafficResetDay:31,consumptionMultiplier:1.0}')"
  fi
  if [[ -n "$NODE_UUID" ]]; then
    api_json "$(api_call PATCH "/nodes/${NODE_UUID}" "$body" "$API_JWT")" >/dev/null 2>&1 || \
      warn "Нода $name уже есть; привяжите inbound'ы в панели, если протоколы не появились."
    return 0
  fi
...
profile_inbound_uuids(){
  local existing
  existing="$(api_json "$(api_call GET /config-profiles/ '' "$API_JWT")")"
  jq -c --arg p "$PROFILE_UUID" '[.response.configProfiles[]? | select(.uuid==$p) | .inbounds[]?.uuid]' <<<"$existing"
}

profile_inbound_uuid_by_tag(){
  local tag="$1" existing
  existing="$(api_json "$(api_call GET /config-profiles/ '' "$API_JWT")")"
  jq -r --arg p "$PROFILE_UUID" --arg t "$tag" '.response.configProfiles[]? | select(.uuid==$p) | .inbounds[]? | select(.tag==$t) | .uuid' <<<"$existing" | head -1
}

create_host(){
  local address="$1" remark="${2:-VLESS Reality}" port="${3:-443}" tag="${4:-VLESS_REALITY}" ib existing body host
  ...
  HOST_UUID="$(jq -r --arg a "$address" --arg r "$remark" --arg cp "$PROFILE_UUID" --arg ib "$ib" \
    '.response[]? | select(.remark==$r and .address==$a and .inbound.configProfileUuid==$cp and .inbound.configProfileInboundUuid==$ib) | .uuid' <<<"$existing" | head -1)"
  [[ -n "$HOST_UUID" ]] && return 0
  body="$(jq -nc --arg cp "$PROFILE_UUID" --arg ib "$ib" --arg a "$address" --arg s "$DOMAIN_REALITY" --arg r "$remark" --argjson p "$port" '{inbound:{configProfileUuid:$cp,configProfileInboundUuid:$ib},remark:$r,address:$a,port:$p,sni:$s,fingerprint:"chrome",securityLayer:"DEFAULT",isDisabled:false,isHidden:false,allowInsecure:false}')"
  host="$(api_json "$(api_call POST /hosts/ "$body" "$API_JWT")")"
  HOST_UUID="$(jq -r '.response.uuid // empty' <<<"$host")"
  [[ -n "$HOST_UUID" ]] || die "Host «$remark» не создан."
}
...
rezzosoft_prepare_node(){
  ...
  log "Дальше откройте конвертер Rezzosoft: $CONVERTER_URL"
}
...
apply_protocols(){
  ...
  if [[ "${MODE:-single}" == panel ]]; then
    ...
    addr="$(node_host_address 2>/dev/null || public_ip)"
  fi
  [[ -n "$addr" ]] || die 'Не удалось определить адрес ноды для Host.'
  ...
  if [[ "${MODE:-single}" != panel ]]; then
    ...
    ok 'Панель обновлена. На сервере ноды выполните: remnawave-manager.sh protocols (или пункт меню).'
  fi
  show_credits
}
...
ask_protocols(){
  local choice
  echo
  echo 'Дополнительные транспорты (Reality TCP/443 через SNI уже будет):'
  echo '  [1] только Reality'
  echo '  [2] Reality + Hysteria2 (UDP/443)'
  echo '  [3] Reality + gRPC (TCP/8443)'
  echo '  [4] Reality + xHTTP (TCP/4443)'
  echo '  [5] все: Reality + Hysteria2 + gRPC + xHTTP'
  if [[ $AUTO_YES -eq 1 ]]; then
    choice="${PROTOCOLS_CHOICE:-1}"
  else
  ...
  case "$choice" in
    ...
    3) ENABLE_GRPC=1 ;;
    4) ENABLE_XHTTP=1 ;;
    5) HYSTERIA2=1; ENABLE_GRPC=1; ENABLE_XHTTP=1 ;;
    *) ;;
  esac
...
install_single(){
  ...
  create_protocol_hosts "$DOMAIN_REALITY"
  setup_panel_web
  install_selfsteal
  install_telegram_alerts
  rezzosoft_prepare_node
...
EOF2
  ...
  rezzosoft_prepare_node
  ok 'Нода (отдельный сервер) установлена. Подключите её в панели по SECRET_KEY.'
}
...
Конвертер мультипротокольных конфигов (Rezzosoft KVN):
  $CONVERTER_URL
...
wizard(){
  ...
  case "$mode" in
    ...
    edge|node)
      ...
      ask ADMIN_EMAIL 'Email для сертификата ACME' '' validate_email
      ask NODE_SECRET_KEY 'SECRET_KEY ноды из карточки панели' ''
      ask_protocols
      ;;
...
Протоколы:
  ...
  $0 protocols              добавить транспорты на уже установленную систему
...
══════════════════════════════════════════════════════════════
  ...
  4) Добавить протоколы: Reality, Hysteria2, gRPC, xHTTP
