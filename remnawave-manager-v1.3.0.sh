...
t(){
  ...
  if [[ "${RW_LANG:-ru}" == en ]]; then
    case "$k" in
      ...
      menu_hint) s='  0 exit   ·   number + Enter runs the action   ·   22 language   ·   25 node transports' ;;
      ...
      d24) s='Refresh Rezzosoft / eGames / DigneZzZ modules; update this script' ;;
      ...
      help_nt_add) s='  %s node-transports add grpc|xhttp|hysteria2|all' ;;
      help_nt_rm) s='  %s node-transports remove grpc|xhttp|hysteria2|all' ;;
      ...
      upd_skip) s='Skipped. Later: menu 24 or bash remnawave-manager.sh self-update' ;;
      upd_fail) s='Could not reach GitHub Latest (network). Try again later.' ;;
      upd_menu) s='update: %s available — item 24 or self-update' ;;
    ...
    case "$k" in
      ...
      menu_hint) s='  0 выход   ·   номер + Enter запускает пункт   ·   22 язык   ·   25 транспорты ноды' ;;
      ...
      d24) s='Скачать свежие модули Rezzosoft / eGames / DigneZzZ; обновить этот скрипт' ;;
      ...
      upd_skip) s='Пропущено. Позже: пункт 24 или bash remnawave-manager.sh self-update' ;;
      upd_fail) s='Не удалось связаться с GitHub Latest (сеть). Попробуйте позже.' ;;
      upd_menu) s='обновление: доступна %s — пункт 24 или self-update' ;;
...
node_transports_menu(){
  ...
  case "$choice" in
    1) set_transport hysteria2 1; sync_node_transports ;;
    2) set_transport grpc 1; sync_node_transports ;;
    3) set_transport xhttp 1; sync_node_transports ;;
    4) set_transport all 1; sync_node_transports ;;
    5) set_transport hysteria2 0; sync_node_transports ;;
    6) set_transport grpc 0; sync_node_transports ;;
    7) set_transport xhttp 0; sync_node_transports ;;
    8) set_transport all 0; sync_node_transports ;;
...
self_update(){
  ...
  dest="$(script_path)"
  tmp="$(mktemp)"
  curl -fL --retry 5 --retry-all-errors --max-time 120 -o "$tmp" "$SELF_UPDATE_URL" \
    || { rm -f "$tmp"; die "Не удалось скачать $SELF_UPDATE_URL"; }
  bash -n "$tmp" || { rm -f "$tmp"; die 'Скачанный скрипт не проходит bash -n.'; }
  ...
  rm -f "$tmp"
  ok "$(t self_ok)"
  exec bash "$dest" ${RW_LANG:+--lang "$RW_LANG"} --no-update-check
}
...
fetch_latest_tag(){
  local json tag loc
  json="$(curl -fsS --connect-timeout 3 --max-time 8 \
    -H 'Accept: application/vnd.github+json' \
    -H 'User-Agent: remnawave-manager' \
  ...
  loc="$(curl -fsSI -L --connect-timeout 3 --max-time 8 \
    -o /dev/null -w '%{url_effective}' \
    "$SELF_UPDATE_URL" 2>/dev/null || true)"
  tag="$(printf '%s' "$loc" | sed -n 's#.*/download/v\([^/]*\)/.*#\1#p')"
  tag="$(normalize_tag "$tag")"
  [[ -n "$tag" ]] || return 1
  printf '%s' "$tag"
}
...
check_self_update(){
  ...
  [[ "$last" =~ ^[0-9]+$ ]] || last=0
  age=$((now - last))
  if [[ "$force" != 1 && "$age" -ge 0 && "$age" -lt ${UPDATE_CHECK_TTL:-21600} && -n "${LAST_REMOTE_VERSION:-}" ]]; then
...
EOF2
  ...
  persist_protocol_flags
  rezzosoft_prepare_node
  ok 'Нода на отдельном сервере запущена. Карточка Node, inbound, Host и сквад создаются API на сервере панели (install panel / пункт 25 / node-transports) — UI панели не нужен.'
}
...
main(){
  ...
  case "${1:-}" in
    ...
    check-update|check-updates) check_update_cli "${2:-}" ;;
