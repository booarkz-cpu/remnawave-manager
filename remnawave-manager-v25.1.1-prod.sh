...
restore(){
  local archive="$1" tmp extracted dbfile stamp pgpass
  ...
  # Для Panel сначала поднимаем только БД/Valkey, затем восстанавливаем SQL dump.
  dbfile="/var/backups/remnawave/remnawave-db-${stamp}.sql"
  if [[ -f "$BASE/docker-compose.yml" && -f "$dbfile" ]]; then
    cd "$BASE"
    ...
    docker exec -i -e PGPASSWORD="$pgpass" remnawave-db psql -U postgres -d postgres -v ON_ERROR_STOP=1 < "$dbfile" >/dev/null || die 'Импорт PostgreSQL dump не выполнен.'
  fi

  for d in "$BASE" "$BASE/subscription" "$BASE/node" "$EDGE_BASE/node" "$BASE/hysteria2" "$BASE/monitoring"; do
    [[ -f "$d/docker-compose.yml" ]] && (cd "$d" && docker compose up -d || true)
  done
  nginx -t && systemctl reload nginx || true
  rm -rf "$tmp"
  trap - RETURN
  ok 'Restore завершён, включая импорт PostgreSQL dump при наличии.'
}
uninstall(){
