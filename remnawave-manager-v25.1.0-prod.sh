...
write_panel_env(){
  ...
  [[ -n "$pgpass" ]] || pgpass="$(rand 40)"
  appsecret="${APP_SECRET:-$(grep '^APP_SECRET=' .env | cut -d= -f2- || true)}"
  [[ -n "$appsecret" && "$appsecret" != change_me ]] || appsecret="$(rand 64)"
  ...
  set_env FRONT_END_DOMAIN "https://${DOMAIN_PANEL}"
...
issue_cert(){
  ...
  if [[ -f "/etc/letsencrypt/live/$domain/fullchain.pem" ]]; then return 0; fi
...
for f in "${compose_files[@]}"; do
  [[ -f "$f" ]] || continue
  if ! docker compose -f "$f" ps --status running --services 2>/dev/null | grep -q .; then
...
db="/var/backups/remnawave/remnawave-db-$stamp.sql"
...
tar --ignore-failed-read -czf "$out" /opt/remnawave /opt/remnawave-edge /etc/nginx/conf.d /etc/nginx/stream.conf /etc/letsencrypt "$db" 2>/dev/null
chmod 600 "$out" "$db" 2>/dev/null || true
...
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Secure Web Service</title><style>body{font:16px system-ui,sans-serif;margin:0;background:#f5f7fa;color:#17202a}main{max-width:760px;margin:12vh auto;padding:32px}h1{font-size:40px}p{line-height:1.6;color:#4d5966}</style></head><body><main><h1>Welcome</h1><p>This website is currently available.</p></main></body></html>
EOF_SITE
      ;;
    business)
      cat > "$site/index.html" <<'EOF_SITE'
...
restore(){
  ...
  tmp="$(mktemp -d)"
  if file "$archive" | grep -qi age; then age -d -i "$BASE/backup-age.key" -o "$tmp/restore.tgz" "$archive"; archive="$tmp/restore.tgz"; fi
  tar -tzf "$archive" >/dev/null || die 'Архив повреждён или не является tar.gz.'
  for d in "$BASE" "$BASE/subscription" "$BASE/node" "$EDGE_BASE/node" "$BASE/hysteria2"; do [[ -f "$d/docker-compose.yml" ]] && (cd "$d" && docker compose down || true); done
  tar -xzf "$archive" -C / 2>/dev/null || true
  for d in "$BASE" "$BASE/subscription" "$BASE/node" "$EDGE_BASE/node" "$BASE/hysteria2"; do [[ -f "$d/docker-compose.yml" ]] && (cd "$d" && docker compose up -d || true); done
  nginx -t && systemctl reload nginx || true
  rm -rf "$tmp"
  ok 'Restore завершён. Проверьте doctor.'
}
...
main(){
  ...
  case "${1:-install}" in
    ...
    core-update) require_root; install_xray_core "${3:-$BASE/node}" "${XCORE_SOURCE:-official}";;
    core-restore) require_root; restore_xray_core "${3:-$BASE/node}";;
