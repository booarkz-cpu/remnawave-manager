...
То же: пункт **24** → **2**, или `bash remnawave-manager.sh self-update`.

Не опрашивать GitHub: `--no-update-check`. Только проверка без скачивания: пункт **24** → **3**.

`self-update` скачивает Latest, проверяет `bash -n`, заменяет этот файл и `/usr/local/bin/remnawave-manager` (если CLI ставили), затем перезапускается.

### Старый скрипт (1.2.x, 1.1.x, 25.2.x, …)

В них нет `check-update`. Скачайте Latest и сверьте SHA256, как в [Быстром старте](#быстрый-старт). Если суммы не совпали — файл не запускайте.

Если раньше включали пункт **19**:

```bash
bash remnawave-manager.sh install-script
```

Так нужно на **обоих** VDS: панели и ноде.
