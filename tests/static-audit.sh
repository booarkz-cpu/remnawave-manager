...
cd "$ROOT"
SCRIPT=remnawave-manager-v1.6.1.sh
...
bash "$SCRIPT" --version | grep -Fq '1.6.1'
set +e
...
grep -Fq '1.6.1' /tmp/rw-lang-glue.txt
...
awk '/^cabinet_nginx_locations\(\)/,/^write_panel_vhosts\(\)/' "$SCRIPT" | grep -Fq 'location = /lk'
awk '/^write_cabinet_env\(\)/,/^install_cabinet\(\)/' "$SCRIPT" | grep -Fq 'sed -n'
