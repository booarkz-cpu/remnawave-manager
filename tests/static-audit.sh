...
echo "[1/5] bash -n"
bash -n remnawave-manager-v25.1.7-prod.sh

echo "[2/5] help"
bash remnawave-manager-v25.1.7-prod.sh --help >/dev/null

echo "[3/5] dry-run single"
sudo bash remnawave-manager-v25.1.7-prod.sh install single --dry-run --yes \
...
sudo bash remnawave-manager-v25.1.7-prod.sh install panel --dry-run --yes \
  DOMAIN_PANEL=panel.example.com DOMAIN_SUB=sub.example.com \
  DOMAIN_REALITY=reality.example.com EDGE_ADDRESS=203.0.113.20 \
  ADMIN_EMAIL=admin@example.com >/dev/null
sudo bash remnawave-manager-v25.1.7-prod.sh install edge --dry-run --yes \
...
grep -F "  remnawave-manager-v25.1.7-prod.sh" SHA256SUMS | sha256sum -c -
