#!/usr/bin/env bash
# ==============================================================================
# Remnawave Manager v25.2.1-prod
# Полностью автоматизированный production-установщик Remnawave (интерфейс на русском).
# Профиль, inbound'ы, ноды, хосты и сквад привязываются через API — панель править не нужно.
#
...
trap 'rc=$?; [[ $rc -eq 141 ]] || die "Ошибка в строке ${BASH_LINENO[0]:-$LINENO} (код $rc). Смотрите $LOG"' ERR
...
Добавленные скрипты и функции
  • Rezzosoft KVN — мультипротокол Reality / Hysteria2 / gRPC / xHTTP
    https://github.com/Rrezzak09VPN/remnanode-VLESS-Reality-Hysteria2
    Конвертер конфигов: $CONVERTER_URL
  • eGamesAPI remnawave-reverse-proxy — панель и нода на разных серверах, SelfSteal, Cloudflare DNS
    https://github.com/eGamesAPI/remnawave-reverse-proxy
  • DigneZzZ remnawave-scripts — CLI, бэкапы, ядро Xray, WARP/Tor, NetBird
    https://github.com/DigneZzZ/remnawave-scripts
...
svc_logs(){
  ...
  echo
  echo 'Укажите контейнер: remnawave-manager.sh logs remnawave'
}
...
ask_protocols(){
  ...
  echo
  echo 'Транспорты (привязка к панели — полностью через API, без ручного редактирования):'
  echo '  [1] только Reality'
  echo '  [2] Reality + Hysteria2 (UDP/443)'
  echo '  [3] Reality + gRPC (TCP/8443)'
  echo '  [4] Reality + xHTTP (TCP/4443)'
  echo '  [5] все: Reality + Hysteria2 + gRPC + xHTTP  (по умолчанию)'
  if [[ $AUTO_YES -eq 1 ]]; then
    choice="${PROTOCOLS_CHOICE:-5}"
  else
    read -r -p 'Ваш выбор [5]: ' choice
    choice="${choice:-5}"
  fi
  case "$choice" in
...
Профиль, inbound'ы, ноды, хосты и сквад привязаны через API. Панель и конвертер править не нужно.
...
wizard(){
  ...
  case "$mode" in
    single|panel)
      ...
      if [[ "$mode" == panel ]]; then
        if [[ -z "${EDGE_ADDRESS:-}" && $AUTO_YES -eq 0 ]]; then read -r -p 'IP ноды (Enter — добавить ноду позже): ' EDGE_ADDRESS; fi
        if [[ -n "${EDGE_ADDRESS:-}" ]]; then
    ...
    edge|node)
      ...
      ask DOMAIN_REALITY 'Домен REALITY / SelfSteal' '' validate_domain
      ask ADMIN_EMAIL 'Email для сертификата ACME' '' validate_email
      ask NODE_SECRET_KEY 'SECRET_KEY ноды из /opt/remnawave/credentials.txt на сервере панели' ''
      ask_protocols
      ;;
...
usage(){
  cat <<EOF2
Remnawave Manager $VERSION — русский production-установщик

Без аргументов открывается меню.

Установка:
  $0 install single          всё на одном сервере (панель + нода)
  $0 install panel           только панель; ноды подключаются отдельно
  $0 install node            только нода на отдельном сервере (синоним: edge)
...
Два VDS:
  ...
  $0 install node --yes PANEL_IP=203.0.113.10 DOMAIN_REALITY=reality.example.com ADMIN_EMAIL=admin@example.com NODE_SECRET_KEY='секрет_ноды'

Протоколы (по умолчанию все; привязка через API, без правок в панели):
  --all-protocols   Reality + Hysteria2 + gRPC + xHTTP (это же значение по умолчанию)
  --reality-only    только VLESS Reality
  --hysteria2       Hysteria2 (Xray inbound UDP/443 + sing-box UDP/8443)
  --grpc            VLESS gRPC + Reality TCP/8443
  --xhttp           VLESS xHTTP + Reality TCP/4443
  $0 protocols      обновить профиль и заново привязать ноды/хосты/сквад
  $0 bind           то же, что protocols

Обслуживание:
  $0 menu | status | doctor | repair | backup | restore FILE | update | uninstall
  $0 up | down | restart | logs [контейнер]
  $0 core-update | core-restore
  $0 stealth                скрытый URL входа в панель (идея eGames)
  $0 install-script         команда remnawave-manager в /usr/local/bin

Модули (авторство сохранено):
  $0 addon remnawave|remnanode|selfsteal|wtm|netbird|egames

Конвертер конфигов Rezzosoft KVN:
  $CONVERTER_URL
EOF2
  show_credits
}
...
══════════════════════════════════════════════════════════════
 Remnawave Manager $VERSION
 Автор: booarkz-cpu  |  добавлены скрипты Rezzosoft, eGames, DigneZzZ
 Конвертер конфигов: $CONVERTER_URL
══════════════════════════════════════════════════════════════
  1) Установить всё на один сервер (панель + нода)
  2) Установить только панель (отдельный сервер)
  3) Установить только ноду и подключить к панели
  4) Автопривязка Reality / Hysteria2 / gRPC / xHTTP (без панели)
  5) Состояние / диагностика
  6) Исправить nginx и подписку (repair)
  7) Логи контейнеров
  8) Перезапуск сервисов
  9) Резервная копия
 10) Обновление
 11) Ядро Xray
 12) Модули DigneZzZ / eGames (WARP, SelfSteal, NetBird)
 13) Открыть конвертер Rezzosoft ($CONVERTER_URL)
 14) Авторство и справка
 15) Скрытый вход в панель (eGames cookie)
  0) Выход
EOF2
    read -r -p 'Выбор: ' choice
    case "$choice" in
      1) require_root; wizard single; install_single; show_credits ;;
      2) require_root; wizard panel; install_panel; show_credits ;;
      3) require_root; wizard node; install_edge; show_credits ;;
      4) require_root; ask_protocols; apply_protocols ;;
      5) doctor; status ;;
      6) require_root; repair ;;
      7) svc_logs "${2:-}" ;;
      8) require_root; compose_action restart; ok 'Сервисы перезапущены.' ;;
      9) require_root; backup ;;
      10) require_root; update ;;
      11) require_root; install_xray_core "${BASE}/node" "${XCORE_SOURCE:-official}" ;;
      12)
        echo '  a) DigneZzZ remnawave CLI'
        echo '  b) DigneZzZ remnanode CLI'
        echo '  c) SelfSteal шаблоны'
        echo '  d) WARP/Tor (wtm)'
        echo '  e) NetBird'
        echo '  f) eGames reverse-proxy'
        read -r -p 'Модуль: ' choice
        case "$choice" in
        ...
        ;;
      13)
        echo "Конвертер Rezzosoft KVN: $CONVERTER_URL"
        command -v xdg-open >/dev/null && xdg-open "$CONVERTER_URL" 2>/dev/null || true
        ;;
      14) usage ;;
      15) require_root; enable_panel_stealth ;;
      0|q|Q) exit 0 ;;
...
main(){
  ...
  case "${1:-}" in
    ...
    install)
      ...
      wizard "$mode"
      if [[ $DRY_RUN -eq 1 ]]; then
        echo "ПРОБНЫЙ ЗАПУСК: режим=$mode panel=${DOMAIN_PANEL:-} sub=${DOMAIN_SUB:-} reality=${DOMAIN_REALITY:-} hysteria2=$HYSTERIA2 grpc=$ENABLE_GRPC xhttp=$ENABLE_XHTTP"
        install_base
        configure_security
        configure_kernel
        write_firewall "$mode"
      ...
      case "$mode" in single) install_single;; panel) install_panel;; edge) install_edge;; *) die "Неизвестный режим: $mode";; esac
      show_credits
      ;;
