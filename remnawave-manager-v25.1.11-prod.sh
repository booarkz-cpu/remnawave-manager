#!/usr/bin/env bash
# ==============================================================================
# Remnawave Manager v25.1.11-prod
...
trap 'rc=$?; die "Ошибка в строке ${BASH_LINENO[0]:-$LINENO} (код $rc). Смотрите $LOG"' ERR
...
ssh_ports(){
  ...
  printf '%s\n' "$ports" | awk 'NF{print}' | sort -nu
}

ssh_port(){ ssh_ports | head -1; }
...
write_firewall(){
  ...
  if [[ "$mode" == single ]]; then
    ...
    if [[ -n "${subnet:-}" ]]; then
      ufw allow from "$subnet" to any port 2222 proto tcp >/dev/null
    else
      ufw allow from 172.16.0.0/12 to any port 2222 proto tcp >/dev/null
    fi
  fi
...
write_sni_router(){
  ...
  if [[ ! -f /var/www/reality-site/index.html ]]; then
