...
При запуске меню **сначала язык** (Enter — текущий), а не только пункт 22. `--lang` и `--yes` выбор пропускают.

Аудит: `KEY=VALUE` в CLI больше не перезаписывает `PATH`/`IFS`/`LD_PRELOAD` (тот же список, что у `manager.env`). `users_cli` передаёт все аргументы после `create`. Имя повторного API-токена без `date | tail` (меньше SIGPIPE). Выбор пользователя/ноды через `10#`. `jq` в списке пользователей не роняет меню на битом JSON.
