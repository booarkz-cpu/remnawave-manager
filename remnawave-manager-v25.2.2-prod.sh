#!/usr/bin/env bash
# ==============================================================================
# Remnawave Manager v25.2.2-prod
...
# Keep console/log output plain-text. Do not emit literal or raw ANSI escape sequences.
...
init_ui_lang(){
  ...
  if n="$(normalize_lang "${LC_ALL:-}")"; then
    RW_LANG="$n"
    return 0
  fi
  if n="$(normalize_lang "${LANG%%.*}")"; then
    RW_LANG="$n"
    return 0
  fi
  RW_LANG=ru
...
t(){
  ...
  if [[ "${RW_LANG:-ru}" == en ]]; then
    case "$k" in
      ...
      err_root) s='Run the script as root.' ;;
      ...
      menu_author) s='Author: booarkz-cpu  |  extras from Rezzosoft, eGames, DigneZzZ' ;;
      menu_langline) s='Language: English  |  item 22 switches to Russian' ;;
      menu_converter) s='Optional converter: %s' ;;
      menu_sec_install) s='INSTALL' ;;
      menu_sec_ops) s='OPERATIONS' ;;
      menu_sec_extra) s='EXTRAS' ;;
      m1) s='  1) Full install (panel + node on one VPS)' ;;
      d1) s='     Panel, node, nginx SNI, Corgi SelfSteal, certificates, API bind of Reality/Hysteria2/gRPC/xHTTP.' ;;
      m2) s='  2) Panel only (separate server)' ;;
      d2) s='     Panel + subscription page + HTTPS. Set EDGE_ADDRESS to register the node now.' ;;
      m3) s='  3) Node only (connect to an existing panel)' ;;
      d3) s='     Host-network remnanode, Reality SNI site, certs. SECRET_KEY comes from panel credentials.txt.' ;;
      m4) s='  4) Auto-bind protocols' ;;
      d4) s='     Refresh AUTO-PROFILE, attach inbounds to nodes, create hosts and AUTO squad. No panel UI.' ;;
      m5) s='  5) Status' ;;
      d5) s='     Running containers, nginx/fail2ban, systemd timers.' ;;
      m6) s='  6) Doctor (diagnostics)' ;;
      d6) s='     Panel API, subscription page, listening ports, UFW.' ;;
      m7) s='  7) Repair nginx and subscription' ;;
      d7) s='     Rewrite proxy headers and SelfSteal without wiping Docker/DB.' ;;
      m8) s='  8) Logs' ;;
      d8) s='     Follow a container (remnawave, remnanode, subscription-page).' ;;
      m9) s='  9) Start services (up)' ;;
      d9) s='     docker compose up for panel, node, subscription, Hysteria2, monitoring.' ;;
      m10) s=' 10) Stop services (down)' ;;
      d10) s='     docker compose down for the same stacks (data is kept).' ;;
      m11) s=' 11) Restart services' ;;
      d11) s='     Restart every Remnawave compose stack.' ;;
      m12) s=' 12) Backup' ;;
      d12) s='     Archive configs and database under /var/backups/remnawave.' ;;
      m13) s=' 13) Restore' ;;
      d13) s='     Restore from a .tgz or .age archive.' ;;
      m14) s=' 14) Update' ;;
      d14) s='     Backup, pull images (panel → node → subscription), verify API.' ;;
      m15) s=' 15) Uninstall' ;;
      d15) s='     Remove services and nginx vhosts; backups are kept.' ;;
      m16) s=' 16) Xray core' ;;
      d16) s='     Install a custom Xray binary or restore the image builtin.' ;;
      m17) s=' 17) Add-ons (DigneZzZ / eGames)' ;;
      d17) s='     remnawave/remnanode CLI, SelfSteal templates, WARP/Tor, NetBird, reverse-proxy.' ;;
      m18) s=' 18) Stealth panel login' ;;
      d18) s='     Cookie/query gate so /auth/login is hidden without a secret (eGames idea).' ;;
      m19) s=' 19) Install CLI command' ;;
      d19) s='     Copy this script to /usr/local/bin/remnawave-manager.' ;;
      m20) s=' 20) Rezzosoft converter' ;;
      d20) s='     Optional JSON helper. Not required for install or API bind.' ;;
      m21) s=' 21) Credits and full help' ;;
      d21) s='     Authorship, CLI flags, install modes.' ;;
      m22) s=' 22) Language / Язык' ;;
      d22) s='     English or Русский. Saved in /opt/remnawave/manager.env.' ;;
      m0) s='  0) Exit' ;;
    ...
    case "$k" in
      ...
      err_root) s='Запустите скрипт от root.' ;;
      ...
      menu_header) s='Remnawave Manager %s' ;;
      menu_author) s='Автор: booarkz-cpu  |  добавлены скрипты Rezzosoft, eGames, DigneZzZ' ;;
      menu_langline) s='Язык: русский  |  пункт 22 переключает на English' ;;
      menu_converter) s='Необязательный конвертер: %s' ;;
      menu_sec_install) s='УСТАНОВКА' ;;
      menu_sec_ops) s='ОБСЛУЖИВАНИЕ' ;;
      menu_sec_extra) s='ДОПОЛНИТЕЛЬНО' ;;
      m1) s='  1) Полная установка (панель + нода на одном VDS)' ;;
      d1) s='     Панель, нода, nginx SNI, сайт Corgi SelfSteal, сертификаты, автопривязка Reality/Hysteria2/gRPC/xHTTP.' ;;
      m2) s='  2) Только панель (отдельный сервер)' ;;
      d2) s='     Панель + страница подписки + HTTPS. EDGE_ADDRESS сразу регистрирует ноду.' ;;
      m3) s='  3) Только нода (подключение к существующей панели)' ;;
      d3) s='     remnanode в host-сети, SNI-сайт Reality, сертификаты. SECRET_KEY из credentials.txt панели.' ;;
      m4) s='  4) Автопривязка протоколов' ;;
      d4) s='     Обновляет AUTO-PROFILE, вешает inbound'\''ы на ноды, создаёт хосты и сквад AUTO. Без UI панели.' ;;
      m5) s='  5) Состояние' ;;
      d5) s='     Контейнеры, nginx/fail2ban, systemd-таймеры.' ;;
      m6) s='  6) Диагностика (doctor)' ;;
      d6) s='     API панели, страница подписки, порты, UFW.' ;;
      m7) s='  7) Исправить nginx и подписку (repair)' ;;
      d7) s='     Переписывает proxy-заголовки и SelfSteal без удаления Docker/БД.' ;;
      m8) s='  8) Логи' ;;
      d8) s='     Журнал контейнера (remnawave, remnanode, subscription-page).' ;;
      m9) s='  9) Запустить сервисы (up)' ;;
      d9) s='     docker compose up для панели, ноды, подписки, Hysteria2, мониторинга.' ;;
      m10) s=' 10) Остановить сервисы (down)' ;;
      d10) s='     docker compose down тех же стеков (данные сохраняются).' ;;
      m11) s=' 11) Перезапуск сервисов' ;;
      d11) s='     Перезапуск всех compose-стеков Remnawave.' ;;
      m12) s=' 12) Резервная копия' ;;
      d12) s='     Архив конфигурации и БД в /var/backups/remnawave.' ;;
      m13) s=' 13) Восстановление' ;;
      d13) s='     Восстановление из архива .tgz или .age.' ;;
      m14) s=' 14) Обновление' ;;
      d14) s='     Backup, pull образов (панель → нода → подписка), проверка API.' ;;
      m15) s=' 15) Удаление' ;;
      d15) s='     Снимает сервисы и nginx vhost; backup-копии остаются.' ;;
      m16) s=' 16) Ядро Xray' ;;
      d16) s='     Поставить свой бинарник Xray или вернуть штатный из образа.' ;;
      m17) s=' 17) Модули DigneZzZ / eGames' ;;
      d17) s='     CLI панели/ноды, шаблоны SelfSteal, WARP/Tor, NetBird, reverse-proxy.' ;;
      m18) s=' 18) Скрытый вход в панель' ;;
      d18) s='     Cookie/query-затвор: /auth/login без секрета отдаёт 404 (идея eGames).' ;;
      m19) s=' 19) Установить команду CLI' ;;
      d19) s='     Копирует скрипт в /usr/local/bin/remnawave-manager.' ;;
      m20) s=' 20) Конвертер Rezzosoft' ;;
      d20) s='     Необязательный помощник JSON. Для установки и привязки не нужен.' ;;
      m21) s=' 21) Авторство и полная справка' ;;
      d21) s='     Авторы, флаги CLI, режимы установки.' ;;
      m22) s=' 22) Язык / Language' ;;
      d22) s='     Русский или English. Сохраняется в /opt/remnawave/manager.env.' ;;
      m0) s='  0) Выход' ;;
...
require_root(){ [[ $EUID -eq 0 ]] || die "$(t err_root)"; }
...
profile_inbound_uuids(){
  ...
  jq -c --arg p "$PROFILE_UUID" '[.response.configProfiles[]? | select(.uuid==$p) | .inbounds[]?.uuid | select(. != null)]' <<<"$existing"
}

profile_inbound_uuid_by_tag(){
  local tag="$1" existing
  existing="$(api_json "$(api_call GET /config-profiles/ '' "$API_JWT")")"
  jq -r --arg p "$PROFILE_UUID" --arg t "$tag" '
...
create_config_profile(){
  [[ -n "${REALITY_SHORT_ID:-}" ]] || REALITY_SHORT_ID="$(rand 16)"
  local existing profile created body
  existing="$(api_json "$(api_call GET /config-profiles/ '' "$API_JWT")")"
  PROFILE_UUID="$(jq -r '[.response.configProfiles[]? | select(.name=="AUTO-PROFILE") | .uuid] | .[0] // empty' <<<"$existing")"
  profile="$(build_xray_config)"
  if [[ -n "$PROFILE_UUID" ]]; then
    # Contract: PATCH /api/config-profiles/ with uuid in the JSON body.
    body="$(jq -nc --arg u "$PROFILE_UUID" --arg n AUTO-PROFILE --argjson c "$profile" '{uuid:$u,name:$n,config:$c}')"
    api_json "$(api_call PATCH /config-profiles/ "$body" "$API_JWT")" >/dev/null
    ok "Профиль AUTO-PROFILE обновлён ($(protocol_summary))."
  else
    created="$(api_json "$(api_call POST /config-profiles/ "$(jq -nc --arg n AUTO-PROFILE --argjson c "$profile" '{name:$n,config:$c}')" "$API_JWT")")"
    PROFILE_UUID="$(jq -r '.response.uuid // empty' <<<"$created")"
    ok "Профиль AUTO-PROFILE создан ($(protocol_summary))."
  fi
  [[ -n "$PROFILE_UUID" && "$PROFILE_UUID" != null ]] || die 'Config Profile UUID не получен.'
  existing="$(api_json "$(api_call GET /config-profiles/ '' "$API_JWT")")"
  INBOUND_UUID="$(jq -r --arg p "$PROFILE_UUID" '
    ...
    // empty
  ' <<<"$existing")"
  [[ -n "$INBOUND_UUID" && "$INBOUND_UUID" != null ]] || die 'AUTO-PROFILE: inbound VLESS_REALITY не найден.'
...
create_node(){
  local address="$1" name="$2" existing body node inbound_uuids plugin_uuid="" plugin_raw=""
  existing="$(api_json "$(api_call GET /nodes/ '' "$API_JWT")")"
  NODE_UUID="$(jq -r --arg n "$name" '[.response[]? | select(.name==$n) | .uuid] | .[0] // empty' <<<"$existing")"
  inbound_uuids="$(profile_inbound_uuids)"
  [[ -n "$inbound_uuids" && "$inbound_uuids" != '[]' ]] || die 'Нет inbound UUID для привязки ноды.'
  ...
  if [[ -n "$NODE_UUID" ]]; then
    ...
    ok "Нода $name обновлена: все inbound'ы AUTO-PROFILE привязаны автоматически."
    return 0
  fi
  ...
  persist_manager NODE_UUID "$NODE_UUID"
  ok "Нода $name создана и привязана к профилю ($(protocol_summary))."
}
...
bind_nodes_to_profile(){
  ...
  if [[ "$node_uuids" == '[]' ]]; then
    ok 'Нод ещё нет — inbound-привязка выполнится, когда появится карточка Node.'
    return 0
  fi
  ...
  while IFS= read -r uuid; do
    [[ -n "$uuid" ]] || continue
    body="$(jq -nc --arg u "$uuid" --arg p "$PROFILE_UUID" --argjson ibs "$inbound_uuids" \
      '{uuid:$u, configProfile:{activeConfigProfileUuid:$p, activeInbounds:$ibs}}')"
    api_json "$(api_call PATCH /nodes/ "$body" "$API_JWT")" >/dev/null || \
      die "Не удалось привязать ноду $uuid. Панель править не нужно — исправьте API и повторите bind."
  done < <(jq -r '.response[]?.uuid // empty' <<<"$existing")
  ok "Каждая нода привязана к AUTO-PROFILE через PATCH /nodes/."
}
...
ensure_internal_squad(){
  local inbound_uuids existing body created uuid
  inbound_uuids="$(profile_inbound_uuids)"
  [[ -n "$inbound_uuids" && "$inbound_uuids" != '[]' ]] || die 'Нет inbound UUID для сквада AUTO.'
  existing="$(api_json "$(api_call GET /internal-squads/ '' "$API_JWT")")"
  SQUAD_UUID="$(jq -r '[.response.internalSquads[]? | select(.name=="AUTO") | .uuid] | .[0] // empty' <<<"$existing")"
  if [[ -n "$SQUAD_UUID" ]]; then
    body="$(jq -nc --arg u "$SQUAD_UUID" --arg n AUTO --argjson ibs "$inbound_uuids" '{uuid:$u,name:$n,inbounds:$ibs}')"
    api_json "$(api_call PATCH /internal-squads/ "$body" "$API_JWT")" >/dev/null || \
      die 'Не удалось обновить сквад AUTO.'
  else
    created="$(api_json "$(api_call POST /internal-squads/ "$(jq -nc --arg n AUTO --argjson ibs "$inbound_uuids" '{name:$n,inbounds:$ibs}')" "$API_JWT")")"
    SQUAD_UUID="$(jq -r '.response.uuid // empty' <<<"$created")"
  fi
  [[ -n "$SQUAD_UUID" && "$SQUAD_UUID" != null ]] || die 'Сквад AUTO не создан.'
  persist_manager SQUAD_UUID "$SQUAD_UUID"
  while IFS= read -r uuid; do
    [[ -n "$uuid" && "$uuid" != "$SQUAD_UUID" ]] || continue
    body="$(jq -nc --arg u "$uuid" --argjson ibs "$inbound_uuids" '{uuid:$u,inbounds:$ibs}')"
    api_json "$(api_call PATCH /internal-squads/ "$body" "$API_JWT")" >/dev/null || \
      warn "Сквад $uuid не обновлён; AUTO уже содержит все inbound'ы."
  done < <(jq -r '.response.internalSquads[]?.uuid // empty' <<<"$existing")
  ok "Сквад AUTO содержит все inbound'ы профиля. Пользователей в панели назначать не нужно — создаётся AUTO-USER."
}
...
create_auto_user(){
  local existing body created expire username='AUTO' raw code json
  [[ -n "${SQUAD_UUID:-}" ]] || return 0
  existing="$(api_call GET "/users/by-username/${username}" '' "$API_JWT" || true)"
  if [[ "$(tail -n1 <<<"$existing")" =~ ^2[0-9][0-9]$ ]]; then
    AUTO_SHORT_UUID="$(sed '$d' <<<"$existing" | jq -r '.response.shortUuid // .response.user.shortUuid // empty')"
    SUBSCRIPTION_URL="$(sed '$d' <<<"$existing" | jq -r '.response.subscriptionUrl // .response.user.subscriptionUrl // empty')"
    ...
    ok "Пользователь AUTO уже есть. Подписка: ${SUBSCRIPTION_URL:-n/a}"
    return 0
  fi
  ...
  json="$(sed '$d' <<<"$raw")"
  if [[ ! "$code" =~ ^2[0-9][0-9]$ ]]; then
    warn "Пользователь AUTO не создан (HTTP ${code:-000}). Хосты, ноды и сквад уже привязаны автоматически."
    return 0
  fi
  ...
  persist_manager AUTO_SHORT_UUID "${AUTO_SHORT_UUID:-}"
  persist_manager SUBSCRIPTION_URL "${SUBSCRIPTION_URL:-}"
  ok "Пользователь AUTO создан. Подписка выдаётся без правок в панели."
}
...
bind_all(){
  ...
  ok "Полная автоматическая привязка готова ($(protocol_summary)). Панель и конвертер не требуются."
}
...
rezzosoft_setup_cron(){
  local domain cron_reboot cron_daily
  domain="$(rezzosoft_cert_domain)"
  [[ -n "$domain" ]] || return 0
  cron_reboot="@reboot cp /etc/letsencrypt/live/${domain}/fullchain.pem /dev/shm/hysteria_cert.pem && cp /etc/letsencrypt/live/${domain}/privkey.pem /dev/shm/hysteria_key.pem && chmod 644 /dev/shm/hysteria_*.pem && sleep 15 && docker restart remnanode"
  cron_daily="0 4 * * * cp /etc/letsencrypt/live/${domain}/fullchain.pem /dev/shm/hysteria_cert.pem && cp /etc/letsencrypt/live/${domain}/privkey.pem /dev/shm/hysteria_key.pem && chmod 644 /dev/shm/hysteria_*.pem && docker restart remnanode"
  if crontab -l 2>/dev/null | grep -q 'hysteria_cert.pem'; then
    ok 'Cron Rezzosoft для сертификатов уже настроен.'
    return 0
  fi
  (crontab -l 2>/dev/null; echo "$cron_reboot"; echo "$cron_daily") | crontab -
  ok 'Cron Rezzosoft: обновление сертификатов после reboot и ежедневно в 04:00.'
}
...
apply_protocols(){
  require_root
  [[ -f "$ENV_FILE" ]] || die 'Сначала выполните install.'
...
install_cli(){
  ...
  ok "CLI установлен: $dest  (запуск без аргументов открывает меню)"
}
...
wait_subscription(){
  ...
  docker ps -a --filter name=remnawave-subscription-page --format 'table {{.Names}}\t{{.Status}}' | tee -a "$LOG" || true
  docker logs --tail 50 remnawave-subscription-page 2>&1 | tee -a "$LOG" || true
  die 'Subscription page не отвечает на 127.0.0.1:3010. Смотрите docker logs remnawave-subscription-page.'
}
...
if [[ -f /opt/remnawave/subscription/docker-compose.yml ]]; then
  ...
  sub_code="$(curl -sS -o /dev/null -w '%{http_code}' --max-time 5 \
    -H 'X-Forwarded-For: 127.0.0.1' \
    -H 'X-Forwarded-Proto: https' \
...
Squad AUTO: ${SQUAD_UUID:-n/a}
User AUTO short UUID: ${AUTO_SHORT_UUID:-n/a}
...
install_single(){
  ...
  create_node "$node_addr" AUTO-SINGLE
  bind_all
  setup_panel_web
  install_selfsteal
  install_telegram_alerts
...
doctor(){
  ...
  curl -sS -o /dev/null -w '3010 HTTP %{http_code}\n' --max-time 5 \
    -H 'X-Forwarded-For: 127.0.0.1' \
    -H 'X-Forwarded-Proto: https' \
    -H "Host: ${DOMAIN_SUB:-localhost}" \
    http://127.0.0.1:3010/ 2>/dev/null || echo '3010 unreachable'
...
repair(){
  [[ -f "$ENV_FILE" ]] || die 'Сначала выполните install: нет /opt/remnawave/manager.env'
  ...
  case "${MODE:-single}" in
    panel)
      [[ -n "${DOMAIN_PANEL:-}" && -n "${DOMAIN_SUB:-}" ]] || die 'repair panel требует DOMAIN_PANEL и DOMAIN_SUB в manager.env'
      write_http_server "$DOMAIN_PANEL" "$DOMAIN_SUB"
      write_panel_vhosts
      write_sni_router 0
      ensure_subscription
      ;;
    edge)
      [[ -n "${DOMAIN_REALITY:-}" ]] || die 'repair edge требует DOMAIN_REALITY в manager.env'
      install_selfsteal
      write_http_server "$DOMAIN_REALITY"
      cat > /etc/nginx/conf.d/reality-site.conf <<EOF2
server {
...
EOF2
      ...
      ;;
    *)
      [[ -n "${DOMAIN_PANEL:-}" && -n "${DOMAIN_SUB:-}" && -n "${DOMAIN_REALITY:-}" ]] || die 'repair single требует DOMAIN_PANEL, DOMAIN_SUB и DOMAIN_REALITY в manager.env'
      write_http_server "$DOMAIN_PANEL" "$DOMAIN_SUB" "$DOMAIN_REALITY"
      write_panel_vhosts
      write_sni_router 1
      install_selfsteal
...
══════════════════════════════════════════════════════════════
 $(t menu_header "$VERSION")
 $(t menu_author)
 $(t menu_langline)
 $(t menu_converter "$CONVERTER_URL")
══════════════════════════════════════════════════════════════
$(t menu_sec_install)
$(t m1)
$(t d1)
...
interactive_menu(){
  ...
  while true; do
    ...
    case "$choice" in
      1) require_root; wizard single; install_single; show_credits; menu_pause ;;
      2) require_root; wizard panel; install_panel; show_credits; menu_pause ;;
      3) require_root; wizard node; install_edge; show_credits; menu_pause ;;
