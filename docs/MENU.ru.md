...
Меню без аргументов: `bash remnawave-manager.sh`. **Сначала язык** (Enter оставляет текущий); пункт **22** переключает позже. Префикс `sudo` не пишите. Номера **1–27** не съезжают; **28–32** добавлены в 1.5.0; **33** — заглушка корги на домене SUB (1.5.5, HTTP 500 на `/` исправлен в 1.5.6). **0** / `q` — выход.
...
nginx 1.24 отдаёт `/` через `root` + `try_files /index.html`. `alias` на `location = /` даёт **HTTP 500** (`index.htmlindex.html` — не каталог). 1.5.6 переписывает такой vhost при следующем запуске от root после `self-update`. Состояние печатает HTTP-код `https://SUB/`.
