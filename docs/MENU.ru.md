...
Меню без аргументов: `bash remnawave-manager.sh`. **Сначала язык** (Enter оставляет текущий); пункт **22** переключает позже. Префикс `sudo` не пишите. Номера **1–27** не съезжают; **28–32** добавлены в 1.5.0; **33** — заглушка корги на домене SUB (1.5.5); **34** — личный кабинет (1.6.0, аудит 1.6.1). **0** / `q` — выход.
