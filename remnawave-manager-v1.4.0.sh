...
pick_target_node(){
  local existing count choice i uuid name address conn st
  local -a uuids names
  [[ "${MODE:-single}" == edge ]] && return 0
  [[ ${AUTO_YES:-0} -eq 1 ]] && return 0
  [[ -n "${TARGET_NODE_UUID:-}" ]] && return 0
  local tok="${API_JWT:-${API_TOKEN:-}}"
  [[ -n "$tok" ]] || return 0
  existing="$(api_json_soft "$(api_call GET /nodes/ '' "$tok" 2>/dev/null || true)" || true)"
  [[ -n "$existing" ]] || return 0
  count="$(jq -r '[.response[]?.uuid | select(. != null and . != "")] | length' <<<"$existing" 2>/dev/null || echo 0)"
  [[ "${count:-0}" =~ ^[0-9]+$ ]] || return 0
  [[ "$count" -gt 1 ]] || return 0
  echo
