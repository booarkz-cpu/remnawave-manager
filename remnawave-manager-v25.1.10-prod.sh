#!/usr/bin/env bash
# ==============================================================================
# Remnawave Manager v25.1.10-prod
...
write_firewall(){
  ...
  if [[ $DRY_RUN -eq 1 ]]; then
    log "[DRY-RUN] UFW: SSH $ssh_list/tcp, 80/tcp, 443/tcp; Edge: 8443/udp (если включена Hysteria2); Node API 2222 только с Panel"
    return 0
  fi
...
bootstrap_auth(){
  ...
  local t token_name="remna-manager-bootstrap"
  [[ ${#token_name} -le 30 ]] || die "Внутренняя ошибка: имя API token длиннее 30 символов."
  t="$(api_json "$(api_call POST /tokens "$(jq -nc --arg n "$token_name" '{name:$n,expiresInDays:3650,scopes:["*"]}')" "$API_JWT")")"
...
create_subscription_token(){
  ...
  local body result
  body="$(jq -nc --arg n "remna-subpage" '{name:$n,expiresInDays:3650,scopes:["subscription-page-configs:list","subscription-page-configs:get","subscriptions:subpage-config","system:metadata","users:by-username"]}')"
