#!/usr/bin/env bash
# ==============================================================================
# Remnawave Manager v25.2.3-prod
...
t(){
  ...
  if [[ "${RW_LANG:-ru}" == en ]]; then
    case "$k" in
      ...
      d4) s='Profile, inbounds, hosts, AUTO squad — no panel UI' ;;
      m5) s='Status' ;;
      ...
      m23) s='URLs' ;;
      d23) s='Panel, subscription, SNI and AUTO user link (not passwords)' ;;
      m0) s='Exit' ;;
    ...
    case "$k" in
      ...
      d4) s='Профиль, inbound, хосты, сквад AUTO — без UI панели' ;;
      m5) s='Состояние' ;;
      ...
      m23) s='Адреса' ;;
      d23) s='Панель, подписка, SNI и ссылка AUTO (без паролей)' ;;
      m0) s='Выход' ;;
...
already_installed(){
  [[ -f "$ENV_FILE" && -f "$BASE/docker-compose.yml" ]]
}
...
show_urls(){
  ...
  if [[ -n "${SUBSCRIPTION_URL:-}" && "$SUBSCRIPTION_URL" != n/a ]]; then
    echo "  AUTO user:    $SUBSCRIPTION_URL"
  elif [[ -n "${AUTO_SHORT_UUID:-}" ]]; then
    echo "  AUTO user:    https://${DOMAIN_SUB}/${AUTO_SHORT_UUID}"
  fi
