...
Полное описание пунктов меню 0–34: [docs/MENU.ru.md](docs/MENU.ru.md) · [docs/MENU.en.md](docs/MENU.en.md).

В `1.6.3` кабинет сразу показывает правки меню из админки. В `1.6.2` админ-кабинет на Material You + Web 3.0. В `1.6.1` аудит кабинета (Telegram widget, правка вкладок, санитайзер HTML). В `1.6.0` личный кабинет пользователя (пункт **34**): вход email/Telegram/VK/Яндекс, тарифы, пробный период, ссылка подписки, инструкции устройств; админ правит меню. Тест без платёжных шлюзов: `remnawave-cabinet-test.sh`.

В `1.5.5` на корне домена подписки — сайт питомника корги с фото (пункт **33**). Ссылки `/shortUuid` остаются рабочими.

В `1.5.4` создание пользователя спрашивает срок, трафик и устройства; язык — до нумерованного меню; журнал и Security на EN/RU.

В `1.5.2` аудит: пункт 13 снова делает restore; `ask` не зависает на EOF; `manager.env` не затирает `VERSION`/`PATH`; пункты 27–32 на ноде не роняют меню.

В `1.5.1` пункт 26 не падает при ошибке сети; тег Latest не читается с CDN без версии; `self-update` умеет jsDelivr.
