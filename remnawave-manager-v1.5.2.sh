...
load_kv_file(){
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    case "$key" in
      VERSION|PATH|HOME|IFS|PWD|OLDPWD|EUID|UID|PPID|TMPDIR|CDPATH|BASH_ENV|ENV|SHELLOPTS|BASHOPTS|BASH_XTRACEFD|LD_PRELOAD|LD_LIBRARY_PATH|LD_AUDIT|BASH_SOURCE|BASH_LINENO|FUNCNAME|PIPESTATUS|BASH_VERSINFO|PS1|PS2|PS4)
        continue ;;
    esac
    val="${val#\"}"; val="${val%\"}"
    val="${val#\'}"; val="${val%\'}"
...
# Menu actions that return 1 (empty list, cancelled pick) must not trip ERR.
menu_call(){ "$@" || true; }
