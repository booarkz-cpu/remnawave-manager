...
t(){
  ...
  if [[ "${RW_LANG:-ru}" == en ]]; then
    case "$k" in
      ...
      menu_hint) s='  0 exit   ·   number + Enter   ·   22 language   ·   25 node transports   ·   26 this script   ·   27 add node' ;;
      ...
      nt_pick_none) s='No nodes in the API yet. Paste a UUID, or 0 for all.' ;;
      nt_pick_uuid) s='Number from the list, paste a UUID, or 0 = all.' ;;
      nt_selected) s='Node: %s (%s)' ;;
      nt_all) s='all nodes' ;;
      prompt_nt_node) s='Node (number / UUID / 0 = all): ' ;;
    ...
    case "$k" in
      ...
      menu_hint) s='  0 выход   ·   номер + Enter   ·   22 язык   ·   25 транспорты ноды   ·   26 этот скрипт   ·   27 добавить ноду' ;;
      ...
      nt_pick_none) s='В API пока нет нод. Вставьте UUID или 0 — все.' ;;
      nt_pick_uuid) s='Номер из списка, UUID или 0 = все.' ;;
      nt_selected) s='Нода: %s (%s)' ;;
      nt_all) s='все ноды' ;;
      prompt_nt_node) s='Нода (номер / UUID / 0 = все): ' ;;
...
self_update(){
  ...
  curl -fL --retry 5 --retry-all-errors --max-time 120 -o "$tmp" "$SELF_UPDATE_URL" \
    || { rm -f "$tmp"; die "$(t err_download "$SELF_UPDATE_URL")"; }
  bash -n "$tmp" || { rm -f "$tmp"; die "$(t err_syntax)"; }
...
check_self_update(){
  ...
  if [[ "$force" != 1 && "$age" -ge 0 && "$age" -lt ${UPDATE_CHECK_TTL:-21600} \
     && -n "${LAST_REMOTE_VERSION:-}" ]] && version_is_newer "$LAST_REMOTE_VERSION" "$VERSION"; then
